...
>title="回顾日期范围"
>abstract="控制在查找符合数据馈送交付条件的事件时，Customer Journey Analytics回顾的时间。<br/>如果事件发生在回看日期范围内，则仍可包括超出频率窗口（特定小时或天）的事件。 是否包含事件取决于以下因素：区段鉴别、会话计算、派生字段转换和维度持久性。<br/>较长的回顾日期范围通常会导致更多事件；较短的范围会导致更好的交付性能。"
...
>id="cja_datafeed_lookback_date_range"
>title="回顾日期范围"
>abstract="控制在查找符合数据馈送交付条件的事件时，Customer Journey Analytics回顾的时间。 此设置类似于Analysis Workspace报表窗口，但存在重要差异。<br/>如果事件发生在回看日期范围内，则仍可包括超出频率窗口（特定小时或天）的事件。 是否包含事件取决于以下因素：区段鉴别、会话计算、派生字段转换和维度持久性。<br/>较长的回顾日期范围通常会导致更多事件；较短的范围会导致更好的交付性能。"
...
1. 在&#x200B;[!UICONTROL **投放**]&#x200B;选项卡的&#x200B;[!UICONTROL **计划**]&#x200B;部分中，选择要创建的馈送类型（实时或回填），然后指定报表时段、频率和其他配置选项：
   ...
   | [!UICONTROL **回顾日期范围**] | 控制在处理数据馈送传递时 Customer Journey Analytics 回顾的时间范围。 默认值为30天。 <p>配置此选项时，请考虑以下重要概念：</p><ul><li>较长的回顾日期范围通常会导致更多事件；较短的回顾日期范围会导致更好的交付性能。</li><li>数据馈送中的回顾日期范围与Analysis Workspace中的报表日期范围相似，但存在[主要差异](/help/components/exports/cja-data-feeds/df-comparison-workspace.md#differences)。 这些差异可能会导致在Workspace报表和数据馈送交付之间出现数据描述差异。 </li><li>此设置不会更改频率窗口（小时或天），该窗口定义要包含在数据馈送输出中的事件的时间范围。 <p>如果事件发生在回顾日期范围内，则仍可以包含这些事件并且这些事件不在该频率范围内，具体取决于以下因素： </p><ul><li>**区段鉴别**：将区段应用于您的数据馈送定义时，回顾日期范围内的任何事件都会确定人员是否符合条件。 区段的容器设置确定范围。 (可能的容器包括：“人员”、“会话”或“事件”。 B2B包括以下附加容器：全球客户、客户、商机、购买团体。)  <p>例如，如果应用名为&#x200B;_购买了_&#x200B;的人员的区段，则一周的回溯将包括过去7天内购买的人员在给定小时或当天（频率窗口）发生的事件。 90天回顾将包括过去90天购买者的活动</p></li><li>**会话计算**：会话边界是使用回顾日期范围内的数据计算的。</li><li>**派生字段转换**：引用容器的任何派生字段函数在数据馈送导出中使用回顾日期范围。</li><li>**Dimension持久性**：如果选择在单个维度上设置持久性，则还需要选择到期时间，以确定维度项在从中设置它的事件之外保持多久。 设置为永久的维度使用回顾日期范围来确定数据馈送导出的资格。  <p>当数据视图中的过期时间设置为以下任一选项时，回顾日期范围会影响维度持久性：</p><ul><li>对于数据馈送定义中使用&#x200B;[!UICONTROL **报告窗口**]&#x200B;作为其到期时间的每个维度，回顾日期范围将成为新的报告窗口。</li><li>对于数据馈送定义中使用&#x200B;[!UICONTROL **自定义时间**]&#x200B;作为其到期时间的每个维度，如果选择的自定义时间超出回顾日期范围，则忽略自定义时间，并将回顾日期范围用于维度到期。<p>有关在数据视图中设置维度的持久性的详细信息，请参阅[持久性组件设置](/help/data-views/component-settings/persistence.md)。</p></li></ul><p>将回顾日期范围设置为等于或大于数据中维度设置的持久性的值。 例如，如果促销活动维度的有效期为30天，且某人两周前点击了该促销活动，则7天的回顾日期范围将不会保留该值。</p></ul> |
