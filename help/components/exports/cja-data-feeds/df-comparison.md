...
* 拼接
  ...
  启用跨设备身份解析，将跨设备的事件链接到单个人员。

* 数据的结构化视图

  在交付的文件中构建数据馈送时，会同时使用结构化数据。 Adobe Analytics数据馈送使用字符串。

* 具有与Analysis Workspace匹配的维度和量度的组件边栏

  使用数据视图中可用的维度和量度。 在Adobe Analytics中，使用预定义的字段和列列表。

* 应用于数据视图的任何区段都会在数据馈送中自动继承

* 区段可以直接应用于数据馈送（除了已应用于数据视图的任何区段之外）

* 馈送与数据视图时区<!-- how did it work in AA? -->匹配

* Parquet投放

  输出现代Parquet文件，该文件本机支持复杂的嵌套和结构化数据。 产品列表表示为结构化数组/嵌套对象。

* 蜂窝样式路径

* 对数据视图中的组件所做的更改会传播到数据馈送
