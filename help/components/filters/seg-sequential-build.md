...
像创建简单的混合级别顺序过滤器或嵌套顺序过滤器一样创建一个过滤器，然后设置 [!UICONTROL 排除] 容器元素的运算符。 下面的示例是一个聚合过滤器，其中三个 [!UICONTROL 点击] 容器被拖到画布上， [!UICONTROL 则] 运算符分配，用于联接容器逻辑，然后排除中间的页面查看容器，以仅包含从序列中的页面A访问页面C的访客。
