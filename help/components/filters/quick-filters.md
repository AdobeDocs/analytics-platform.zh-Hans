...
使用快速过滤器，您可以轻松地浏览给定项目中的数据，而无需在 [过滤器生成器](/help/components/filters/create-filters.md).

创建快速过滤器时，请考虑以下事项：

* 快速过滤器仅适用于创建这些过滤器的项目。 此类过滤器在其他项目中不可用，无法共享给其他用户。 
* 最多允许3个规则。
* 不支持嵌套容器或顺序规则。

以下视频演示了如何使用快速过滤器。 (注：此视频使用术语“快速区段”，而不是“快速过滤器”。 但是，功能是相同的。)
...
1. 选择以下方法之一以开始创建快速过滤器：

   * **临时（拖放）：** 从左边栏，将组件拖到 **过滤器** 图标，然后选择 **编辑** 图标来调整过滤器。

      ![编辑临时过滤器](assets/filter-adhoc-edit.png)

      >[!NOTE]
      >
      > 在创建临时快速过滤器时（拖放），请考虑以下事项：
      > * 不支持以下组件类型：计算量度和维度，以及无法从中构建过滤器的量度。
...
1. 调整以下任意设置：
   ...
   | [!UICONTROL 组件]（维度/量度/日期范围） | 通过添加组件（维度、量度、日期范围或维度值）可定义最多 3 条规则。有 3 种方法可以找到正确的组件：<ul><li>开始键入内容，快速过滤器生成器会自动找到相应的组件。</li><li>使用下拉列表查找组件。</li><li>从左边栏中拖放组件。</li></ul> |
   ...
   | 预览（右上角） | 让您查看您的过滤器是否有效以及该过滤器的作用范围。 表示在应用此过滤器时，您可能会看到的数据集的划分。 您可能会收到一条通知，表明此过滤器没有数据。在这种情况下，可继续操作或更改过滤器定义。 |

1. 选择 [!UICONTROL **应用**] 以保存更改。

## 编辑快速过滤器 {#edit}

1. 将鼠标悬停在要编辑的快速过滤器上，然后选择 **编辑** 图标。
...
## 将快速过滤器另存为组件列表过滤器 {#save}

>[!IMPORTANT]
>
> 保存快速过滤器时，请考虑以下事项：
> 
> * 要保存快速过滤器，您需要在 [Adobe Admin Console](https://experienceleague.adobe.com/docs/analytics/admin/admin-console/permissions/summary-tables.html#analytics-tools?lang=zh-Hans).
> 
> * 保存或应用过滤器后，便无法再在快速过滤器生成器中对其进行编辑。 您而是必须使用常规过滤器生成器。


您可以选择将快速过滤器另存为组件列表过滤器。 组件列表过滤器的优势包括：

* 适用于您所有工作区项目
* 支持更复杂的过滤器以及顺序过滤器

您可以从快速过滤器生成器或 [!UICONTROL 过滤器生成器].
...
1. 选择&#x200B;**[!UICONTROL 保存]**。
   ...
   过滤器现在显示在左边栏的组件列表中。 另请注意，滤镜的侧栏从浅蓝色变为深蓝色，这表示它不能再在快速滤镜生成器中编辑或打开。
...
1. （可选）重命名过滤器，然后选择 [!UICONTROL **应用**].
   ...
   返回到工作区，并注意到滤镜的侧栏从浅蓝色变为深蓝色，这表示该滤镜不能再在快速滤镜生成器中编辑或打开。 通过保存它，它就成为组件列表的一部分。
