...
分析需要上下文，并且以前的时间段通常会提供该信息。 例如，问题&#x200B;*与去年的这个时候相比，您现在的表现好或坏多少？* 对于了解您的业务至关重要。 日期对比功能会自动包含一个&#x200B;*差异*&#x200B;列，用于显示与指定时段相比的百分比变化。

1. 创建一个[自由格式表格](/help/analysis-workspace/visualizations/freeform-table/freeform-table.md)，自由选择您希望在指定时段内进行对比的维度和量度。
1. 要确定比较时间范围以及它是滚动时间比较还是固定时间比较，请在面板或列上设置时间段。
...
您现在可以向表中的每一列添加一个时间段。 这使您能够添加与日历所设置的时间段不同的时间段。
...
例如，您可以对上一周（截至 2024 年 10 月 5 日）与前一周进行按日对比分析。 默认情况下，左列从9月22日开始，右列从9月29日开始。
...
* 默认情况下，系统会为所有新项目启用此设置。

* 此设置适用于整个表格。 例如，如果您在表格中的某个细分中更改此设置，该设置将应用于整个表格。

* 启用此设置后，所有列单元格的右上角都会显示小日期标签，以便用相应的日期（和时间，如果相关）在单元格中添加注释。
