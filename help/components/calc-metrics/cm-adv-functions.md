...
[!UICONTROL 置信度] 是一个概率度量，用于衡量是否有多少证据表明给定的变量与控制变量相同。 置信度越高，表明支持控制变量和非控制变量具有相同性能的假设的证据越少。
...
| 成功量度 | 用户正在与变量进行比较的一个或多个量度。 |
| 控制 | 实验中所有其他变体正在与之进行比较的变体。 输入控制变体维度项目的名称。 |
| 显着性阈值 | 此函数中的阈值设置为默认的95%。 |
...
返回在转化中特定变体对控制变体的提升。 它是指给定变体与基线之间的性能差异除以基线的性能（以百分比表示）。
