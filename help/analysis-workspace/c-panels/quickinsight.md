...
| [!UICONTROL 量度] | 量度是有关人员活动的量化信息，包括查看次数、点进次数、重新载入、平均逗留时间、件数、订购、收入等。 |
...
| [!UICONTROL 过滤器] | 过滤器让您可以根据用户特征或网站交互情况来识别用户子集。 例如，您可以构建 [!UICONTROL 访客] 根据属性（例如，浏览器类型、设备、访问次数、国家/地区、性别）或根据交互情况（例如，促销活动、关键词搜索、搜索引擎），或根据进入和退出(例如，来自Facebook的人员、已定义的登陆页面、反向链接域)，或根据自定义变量（例如，表单字段、已定义的类别、客户ID）进行过滤。 |
