...
>title="试验"
>abstract="创建一个面板以比较不同的用户体验、营销或消息传递变量。 并决定哪种变体最能推动特定结果。"
...
>title="试验"
>abstract="比较不同的用户体验、营销或消息变化，以确定哪些最能推动特定结果。<br/><br/>**参数&#x200B;**<br/>**试验**：将分析的试验。<br>**控制变量**：选定试验的控制变量。<br/>**成功量度**：最多5个用于对照其分析试验的标准（非计算）成功量度。<br/>**标准化量度**：人员、会话或事件。 此指标（也称为计数方法）将作为提升计算的分母。此量度还会影响在应用置信度计算之前汇总数据的方式。"
...
试验面板可供所有Customer Journey Analytics用户使用。 不需要管理员权限或其他权限。但是，[先决条件](#prerequisites)需要只有管理员才能执行的操作。
...
1. **试验有结论**：每次查看试验报告时，都会分析到目前为止在试验中积累的数据。 对于&#x200B;*至少一个*&#x200B;变量，当随时有效置信度超过95%的阈值时，分析将宣布试验具有结论性。 对于双臂以上的情况，采用Benjamini-Hochberg校正方法对多重假设检验进行校正。

2. **最佳性能变量**：当一项试验被宣布为具有结论性时，具有最高转化率的变量会被标记为最佳性能变量。 请注意，此变量必须是控制变量或基线变量，或者是超过95%随时有效置信阈值的变量之一（应用Benjamini-Hochberg校正）。
