...
流量可视化图表可帮助您了解源自或导致您的网站或应用程序上的特定转化事件的历程。 它通过维度（和维度项）或量度跟踪路径。
...
1. 在项目中添加一个空白面板，选择左边栏中的可视化图标，然后拖动 [!UICONTROL **流量**] 可视化图表显示在面板中。

   或

   通过中的“将可视化图表添加到面板”部分中所述的任何方式添加可视化图表 [可视化概述](/help/analysis-workspace/visualizations/freeform-analysis-visualizations.md).
...
1. 使用以下选项之一锚定流量可视化图表：
   ...
   * 使用下拉菜单选择量度或维度。
   * 从左边栏拖动维度或量度。
   * 开始键入维度或量度的名称，然后在此维度或量度出现在下拉列表中时将其选定。
...
1. （可选）选择 **[!UICONTROL 显示高级设置]** 配置以下任意选项：
   ...
   | **[!UICONTROL 限制到第一次/最后一次发生次数]** | 将路径限制为以维度/项目/量度的第一次/最后一次发生次数开始/结束的路径。请参阅以下部分， [“仅限于第一次/最后一次发生”的示例场景](#example-scenario-for-limit-to-firstlast-occurrence)，以了解更详细的解释。 |
   | **[!UICONTROL 列数]** | 流量图中所需的列数。 |
   | **[!UICONTROL 每列扩展的项]** | 每列中所需的项目数。 |
...
>[!INFO]
>
>**示例：** 假设您要跟踪用户访问和访问您网站上最受欢迎页面的路径。
>
>要执行此操作，您需要
>1. 如上所述，开始创建流量可视化图表。
>1. 拖动 [!UICONTROL **页面**] 将维度移入 **[!UICONTROL 包含]** 字段，然后选择 [!UICONTROL **生成**].
>1. 流量可视化图表在构建时会在可视化图表中心的焦点节点中显示查看次数最多的页面。 您还可以看到进入该页面的顶部页面（焦点节点左侧），以及退出该焦点页面的顶部页面（焦点节点右侧）。
>1. 分析流量中的数据，如中所述 [查看和更改流量输出](#view-and-change-the-flow-output).
...
| [!UICONTROL 重新开始] | 返回到自由格式表格生成器，您可从中生成新的流量图表。 |
| [!UICONTROL 为此路径创建过滤器] | 创建过滤器。这会将您带入过滤器生成器，您可以在其中配置新过滤器。 |
| [!UICONTROL 细分] | 按可用维度、量度或时间分解节点。 |
| [!UICONTROL 筛选列] | 显示的过滤器选项与在自由格式表格中可用的选项相同。 有关可用选项的更多信息，请参阅中的“将简单或高级过滤器应用到表”部分 [筛选和排序表](/help/analysis-workspace/visualizations/freeform-table/filter-and-sort.md). |
| [!UICONTROL 排除项目]/[!UICONTROL 还原排除的项目] | 从列中删除特定节点，并将其自动创建为列顶部的过滤器。要恢复排除项，请再次右键单击并选择&#x200B;**[!UICONTROL 恢复排除项]**。您也可以打开列顶部的过滤器，移除带有您刚刚排除的项目的 Pillbox。 |
| [!UICONTROL 趋势] | 为节点创建趋势图表。 |
| 显示下一列/显示上一列 | 显示可视化图表的下一（右）列或上一（左）列。 |
| 隐藏列 | 隐藏可视化图表中的选定列。 |
...
* 使用的系列因容器而异。如果使用 **[!UICONTROL 访问]** 容器，则一系列事件将成为会话。 如果使用 **[!UICONTROL 访客]** 容器，则事件系列将是给定用户在提供的日期范围内的所有事件。
...
如果“限制到第一次/最后一次发生次数”被禁用，那么这一系列事件将计为“添加到购物车”的2次发生次数。
预期流量输出：
“添加到购物车” (2) —>“产品” (1)
->“计费” (1)

但是，如果启用了“限制到第一次/最后一次发生次数”，则分析中将只包括“添加到购物车”的第一次发生次数。
预期流量输出：
“添加到购物车” (1) —>“产品” (1)
...
如果 **[!UICONTROL 限制到第一次/最后一次发生次数]** 是 *已禁用*，则这一系列事件将计为“添加到购物车”的2次发生次数。
预期流量输出：
“产品” (2) &lt;— “添加到购物车” (2)
