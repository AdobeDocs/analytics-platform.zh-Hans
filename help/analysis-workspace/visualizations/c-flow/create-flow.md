...
1. 在项目中添加一个空白面板，选择左边栏中的可视化图表图标，然后拖动 [!UICONTROL **流量**] 可视化图表显示在面板中。

   或

   采用中“将可视化图表添加到面板”部分中所述的任何方式添加可视化图表 [可视化概述](/help/analysis-workspace/visualizations/freeform-analysis-visualizations.md).
...
1. （可选）选择 **[!UICONTROL 显示高级设置]** 配置以下任一选项：
   ...
   >[!IMPORTANT]
   >
   >以下项的组合 **[!UICONTROL 列数]** 和 **[!UICONTROL 每列扩展的项]** 确定创建流量可视化图表所需的底层请求数。 这些数字越大，呈现可视化所需的时间就越长。
...
>[!INFO]
>
>**示例：** 假设您要跟踪用户在您的网站上最受欢迎页面之间切换的路径。
>
>要执行此操作，您需要
>
>1. 如上所述，开始创建流量可视化图表。
>1. 拖动 [!UICONTROL **页面**] 将维度移入 **[!UICONTROL 包含]** 字段，然后选择 [!UICONTROL **生成**].
>1. 流量可视化图表在构建时会在可视化图表中心的焦点节点中显示查看次数最多的页面。 您还可以看到进入该页面的顶部页面（焦点节点的左侧）以及退出该焦点页面的顶部页面（焦点节点的右侧）。
...
| [!UICONTROL 为此路径创建过滤器] | 创建过滤器。这会将您转到过滤器生成器，您可以在其中配置新过滤器。 |
| [!UICONTROL 细分] | 按可用维度、量度或时间分解节点。 |
| [!UICONTROL 筛选列] | 显示的过滤器选项与在自由格式表格中可用的选项相同。 有关可用选项的更多信息，请参阅中的“将简单或高级过滤器应用到表”部分 [对表格进行过滤和排序](/help/analysis-workspace/visualizations/freeform-table/filter-and-sort.md). |
| [!UICONTROL 排除项目]/[!UICONTROL 恢复排除的项目] | 从列中删除特定节点，并将其自动创建为列顶部的过滤器。要恢复排除项，请再次右键单击并选择&#x200B;**[!UICONTROL 恢复排除项]**。您也可以打开列顶部的过滤器，移除带有您刚刚排除的项目的 Pillbox。 |
...
如果 **[!UICONTROL 仅限于第一次/最后一次发生]** 是 *已禁用*，则这一系列事件将计为“添加到购物车”的2次发生次数。
预期流量输出：
“产品” (2) &lt;— “添加到购物车” (2)
