...
1. 在项目中添加一个空白面板，选择左边栏中的“可视化”图标，然后将&#x200B;[!UICONTROL **流量**]&#x200B;可视化拖动到面板中。

   或

   使用[可视化概述](/help/analysis-workspace/visualizations/freeform-analysis-visualizations.md)中的“将可视化图表添加到面板”部分中描述的任意方式添加可视化图表。
...
1. （可选）选择&#x200B;**[!UICONTROL 显示高级设置]**&#x200B;以配置以下任一选项：
   ...
   ![显示选项、列数和流量容器的高级设置。](assets/adv-settings.png)
...
| [!UICONTROL 为此路径创建筛选器] | 创建过滤器。这会将您转到过滤器生成器，您可以在其中配置新过滤器。 |
...
| [!UICONTROL 排除项]/[!UICONTROL 还原排除项] | 从列中删除特定节点，并将其自动创建为列顶部的过滤器。要恢复排除项，请再次右键单击并选择&#x200B;**[!UICONTROL 恢复排除项]**。您也可以打开列顶部的过滤器，移除带有您刚刚排除的项目的 Pillbox。 |
...
如果&#x200B;**[!UICONTROL 限制到第一次/最后一次发生次数]**&#x200B;为&#x200B;*禁用*，则这一系列事件将计为“添加到购物车”的2次发生次数。
预期流量输出：
“产品” (2) &lt;— “添加到购物车” (2)
