...
1. 将量度、维度、维度项、过滤器或日期范围从左边栏拖到画布上。 支持基于[派生字段](/help/data-views/derived-fields/derived-fields.md)的量度。 但是，不支持计算指标以及基于[摘要数据集](/help/data-views/summary-data.md)的任何指标或维度。
   ...
   | 维度 | 画布的空白区域 | 为组件被删除的前3个维项创建3个节点，这些节点不与任何现有节点连接。 (**注意：**&#x200B;如果只显示1个或2个节点，则意味着数据仅可用于其中一个或2个维度项。 如果未显示节点，则表示任何维项均无数据可用。 在这种情况下，请尝试将其添加到历程的其他点，调整可视化的日期范围，或选择其他维度。)<p>按住Shift键，将维度拖放到画布上以将其添加为具有3个维度项的单个节点。</p><p></p> |
   ...
   | 过滤器 | 画布的空白区域 | 此时将显示一个节点，其中的组件未与任何其他节点连接。<p>节点上显示的数量和百分比包括按所选过滤器过滤的主指标总数。</p> <p>例如，如果选择“人员”作为历程的主要量度，则向画布的空白区域添加一个过滤器“今天”时，将显示今天发生事件的所有人员。</p> |
   | 过滤器 | 现有节点 | 将过滤器应用于现有节点。 |
   | 过滤器 | 连接2个节点的箭头 | 该节点显示在两个现有节点之间，其中组件被放置并连接到两个现有节点。 （有关详细信息，请参阅[连接节点](#connect-nodes)。）</p><p>将过滤器应用于路径上组件放置到的点。</p> |
   ...
   | 日期范围 | 连接2个节点的箭头 | 该节点显示在组件被删除并连接到两个现有节点的两个现有节点之间。 （有关详细信息，请参阅[连接节点](#connect-nodes)。）</p><p>将日期范围应用于路径上组件被删除的点。</p> |
...
1. 选择要显示维度项目的位置：
   ...
   * [!UICONTROL **在Journey画布中**]：将前3个节点添加到画布中，这些节点位于2个现有节点之间。 每个节点作为画布上的单独分支连接到周围的节点。

   * [!UICONTROL **在自由形式表格中**]：创建自由形式表格可视化内容，其中显示两个现有节点之间的所有顶级维度项。
...
旅程不能是循环的，而是循环回以前连接的节点。

要在Journey Canvas中连接节点，请执行以下操作：
...
1. 选择要用于划分的维度。
   ...
   如果选择在自由形式表中查看细分，则顶级维项将显示在Journey Canvas可视化紧邻上方的新自由形式表中。

#### 将划分应用于单个节点

您可以将维从左边栏拖动到画布上要应用细分的节点上。
