...
连接历程画布中的节点时，使用THEN运算符连接它们。 这也称为[顺序分段](/help/components/filters/seg-sequential-build.md)。
...
从历程画布创建的区段使用[顺序区段](/help/components/filters/seg-sequential-build.md)。 这意味着该区段使用THEN运算符将人员流经的事件序列（历程）链接到选定的节点或箭头。 与选定节点或箭头匹配的所有事件都包括在区段中。
...
1. 指定区段的标题并进行任何其他更改。 有关创建区段的详细信息，请参阅[区段生成器](/help/components/filters/filter-builder.md)。
