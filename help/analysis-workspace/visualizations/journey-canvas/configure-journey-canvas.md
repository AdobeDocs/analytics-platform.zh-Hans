...
1. 将量度、维度、维度项、过滤器或日期范围从左边栏拖到画布上。 不支持计算量度。 此外，不支持任何基于[摘要数据集](/help/data-views/summary-data.md)的量度或维度。
   ...
   | 多个组件 | 画布的空白区域 | **如果没有组件是维度：**<p>每个组件都显示为单独的节点，其中组件被放置，且未与任何现有节点连接。</p><p>按住Shift键，将组件拖放到画布上以将其添加为一个组合节点。 </p><p>**如果您要添加的任何组件是维度：**</p><p>每个组件都显示为单独的节点，其中组件被放置，且未与任何现有节点连接。</p><p>一次只能添加一个维度，并且为删除组件的排名前3的维度项目创建3个节点。</p><p>按住Shift键，将组件拖放到画布上以将其添加为一个组合节点。 排名前3的维度项目将与每个节点相结合。 （有关详细信息，请参阅[合并节点](#combine-nodes)。）</p> |
   | 多个组件 | 现有节点 | 所有组件都与现有节点相结合。<p>如果要添加的任何组件都是维度，则排名前3的维度项将与节点合并。</p> <p>一次只能添加一个维度。</p> |
   | 多个组件 | 连接2个现有节点的箭头 | **如果没有组件是维度：**<p>每个组件都显示为单独的节点，其中放置了组件，并且每个节点都连接到两个现有节点。 （有关详细信息，请参阅[连接节点](#connect-nodes)。）</p><p>按住Shift键，将组件拖放到画布上以将其添加为一个组合节点。 （组件必须属于相同类型才能合并到单个节点中。） （有关详细信息，请参阅[合并节点](#combine-nodes)。）</p><p>**如果您要添加的任何组件是维度：**</p><p>每个组件都显示为单独的节点，其中放置了组件，并且每个节点都连接到两个现有节点。</p><p>一次只能添加一个维度，并为该维度的前3个项目创建3个节点，这些项目位于第一个节点（最终到达第二个节点的人员/会话数）之后的第一个事件之后。 每个节点都连接到两个现有节点。 （有关详细信息，请参阅[连接节点](#connect-nodes)。）</p><p>按住Shift键，将组件拖放到画布上以将其添加为一个组合节点。 排名前3的维度项目与每个节点相结合，每个节点均连接到两个现有节点。 （有关详细信息，请参阅[合并节点](#combine-nodes)。）</p> |
...
您可以选择一个节点，并在旅程中添加该节点之后的前3个节点。

1. 右键单击要在其中添加历程中位于其后的前3个节点的节点。

   此节点在历程中不能有退出它的任何现有节点。

1. 选择&#x200B;[!UICONTROL **在此节点**]&#x200B;之后添加顶级节点。

   旅程中位于此节点之后的前3个节点会被添加，每个节点都会连接到您作为单独分支选择的节点。

#### 在现有节点之前添加顶级节点

您可以添加位于历程中现有节点之前的前3个节点。

1. 右键单击要在其中添加历程中位于其前面的前3个节点的节点。

   此节点在历程中不能有任何现有节点进入它。

1. 选择&#x200B;[!UICONTROL **在此节点**]&#x200B;之前添加顶级节点。

   旅程中位于此节点之前的前3个节点会被添加，每个节点都会连接到您作为单独分支选择的节点。

#### 在现有节点之间添加顶级节点

您可以添加介于2个现有节点之间的前3个节点：

1. 右键单击要在历程中添加前3个节点的2个节点之间的箭头。

1. 选择&#x200B;[!UICONTROL **添加顶级节点**].<!-- I don't think this should have the word "next" in the UI option, because it's both next and previous. It's in between. Just "Get top nodes" sounds better to me.-->
...
从历程画布创建的筛选器使用[连续筛选](/help/components/filters/seg-sequential-build.md)。 这意味着过滤器使用THEN运算符将人员流经的事件（即旅程）序列链接到一起，向上到达选定的节点或箭头。 与选定节点或箭头匹配的所有事件都包含在过滤器中。
