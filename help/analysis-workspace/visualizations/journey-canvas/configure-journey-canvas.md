...
1. （可选）选择&#x200B;[!UICONTROL **显示高级设置**]，然后指定以下信息：
   ...
   | 字段 | 功能 |
   |---------|----------|
   | [!UICONTROL **历程画布容器**] | 选择要在整个历程中重点关注的容器。 您选择的容器决定了在历程中捕获的数据的范围。 这会影响可视化图表中显示的统计数据。 （如果您的容器名称与下面显示的默认名称不同，说明它们已在您的数据视图中自定义过。）<ul><li>**会话：**&#x200B;将可视化图表的统计数据限制在某个给定人员的一个已定义会话内。 这意味着，每个节点上显示的（基于主要和辅助量度的）数字和百分比必须在每个人的某一个会话中发生。 换句话说，一个人可以在一个历程中多次出现。<p>此容器使用“会话”量度。</p></li><li>**人员：**（默认）允许可视化图表的统计数据跨越某个给定人员的多个会话。 这意味着每个节点上显示的（基于主要和辅助量度的）数字和百分比可以在任意数量的会话中发生，只要这些会话属于同一个人。 换句话说，一个人在一次历程中只能出现一次。<p>此容器使用“人员”量度。</p></li></ul> |
...
1. 配置在可视化图表顶部显示的以下任何设置：
   ...
   | [!UICONTROL **与**]&#x200B;比较 | 用于将当前历程数据与前一个时段进行比较的日期范围。 您可以选择以下任意日期范围进行比较：<ul><li>**[!UICONTROL 4周前]**</li><li>**[!UICONTROL 之前2个季度]**</li><li>**[!UICONTROL 1年前]**</li><li>**[!UICONTROL 自定义日期范围]**</li></ul><p>如果您选择了比较日期范围，历程中的每个节点就会根据主要量度显示当前日期范围与选定的比较日期范围之间的百分比变化。 这让您能够确定历程与上一个时段相比是否表现更好。</p> |
   ...
   | **控件** | 以下控件在画布的右上角可用：<ul><li>**适合屏幕** ![适合屏幕图标](assets/fill-screen-icon.png)：调整当前缩放和平移设置，让完整的可视化图表填满屏幕。</li><li>**组织** ![组织图标](assets/organize.svg)：根据节点连接重新排列节点以最小化交叉箭头并优化间距。 </li><li>**放大** ![放大图标](assets/zoom-in-icon.png)：放大可视化图表的特定区域。<p>您也可以使用鼠标控制，例如在触控板上捏合。</li><li>**缩小** ![缩小图标](assets/zoom-out-icon.png)：缩小可视化图表，在画布上腾出更多空间。<p>您也可以使用鼠标控制，例如在触控板上捏合。</p></li></ul><p>要在缩放后在画布上平移，请单击鼠标并拖动到所需的位置。</p> |
...
1. 将量度、维度、维度项、区段或日期范围从左边栏拖到画布上。 基于[派生字段](/help/data-views/derived-fields/derived-fields.md)的量度受到支持。 但是，不支持计算量度以及基于[摘要数据集](/help/data-views/summary-data.md)的任何量度或维度。
   ...
   | 维度 | 画布的空白区域 | 在放置组件的地方为前 3 名维度项创建 3 个节点，不与任何现有节点连接。 （**请注意：**&#x200B;如果只显示 1 个或 2 个节点，就表示数据仅可用于 1 个或 2 个维度项。 如果不显示任何节点，则表示任何维度项都没有可用数据。 在这种情况下，尝试将其添加到历程的另一个点，调整可视化图表的日期范围，或选择另一个维度。）<p>将维度放到画布上时按住 Shift 键，将其添加为具有 3 个维度项的一个节点。</p> |
   | 维度 | 现有节点 | 显示前 5 名维度项的节点会自动应用细分。<!--what happens if you hold Shift?--><p>要在新的可视化自由格式表中查看细分，请选择节点上的&#x200B;[!UICONTROL **在自由格式表中打开**]&#x200B;链接。</p> |
   | 维度 | 连接 2 个现有节点的箭头 | 为（最终到达第二个节点的人员/会话的）第一个节点之后第一个事件后的前 3 名维度项创建 3 个节点。 放置组件的地方会显示这些节点，位于两个现有节点之间，每一个都与这两个现有节点连接。 （**请注意：**&#x200B;如果只显示 1 个或 2 个节点，就表示数据仅可用于 1 个或 2 个维度项。 如果不显示任何节点，则表示任何维度项都没有可用数据。 在这种情况下，尝试将其添加到历程中的其他位置，调整可视化图表的日期范围，或选择其他维度。)<p>将维度放到画布上时按住 Shift 键，将其添加为具有 3 个维度项的一个节点。 （更多信息请参阅[连接节点](#connect-nodes)。）</p> |
...
您可以根据画布上已存在的节点，自动显示排名最前的即时节点或最终节点。 您可以将最顶级的节点添加到历程画布或在自由格式表中查看它们。
...
您可以选择节点，并显示历程中紧随其后的排名最前的直接或最终维度项目。 您可以将前 3 名维度项作为单独的节点添加到历程画布中，也可以在自由格式表中查看所有最顶级的维度项。
...
1. 选择是显示立即出现还是最终出现在所选节点之后的维度项目：

   * [!UICONTROL **紧跟在**]&#x200B;之后：显示直接位于选定节点之后的排名最前的维度项。 当您想知道在历程中的给定步骤后立即发生什么时，使用此项。 例如，您可以查看查看访客在访问给定的产品页面后立即访问的页面。

   * [!UICONTROL **最终晚于**]：显示历程中任何时间点晚于选定节点的排名最前的维度项目，不一定是下一个步骤。 当所选节点是分析中的关键事件，并且您想要查看用户随后在任何时候执行的操作时，可使用此选项。 例如，购买后，您可以看到客户最终倾向于在历程的任意稍后时间访问的排名最前的页面。

1. 选择您想在哪里显示维度项：

   * [!UICONTROL **在历程画布中**]：将前 3 名节点添加到历程中此节点之后的画布中。 每个节点都作为画布上的单独分支连接到您选择的节点。
...
您可以选择节点，并显示历程中位于其前面的排名最前的直接或最终维度项目。 您可以将前 3 名维度项作为单独的节点添加到历程画布中，也可以在自由格式表中查看所有最顶级的维度项。
...
1. 选择是显示立即出现还是最终出现在所选节点之前的维度项目：

   * [!UICONTROL **紧靠在**]&#x200B;之前：显示直接位于选定节点之前的排名最前的维度项。 当您想知道在历程中的给定步骤之前发生的情况时，请使用此项。 例如，您可以查看用户在访问给定的产品页面之前立即访问的页面。

   * [!UICONTROL **最终早于**]：显示历程中任何时间点位于所选节点之前的排名最前的维度项目，不一定是上一步。 当所选节点是分析中的关键事件，并且您希望查看访客在分析之前的任何时间点做了什么操作时，可使用此选项。 例如，在购买之前，您可以查看访客在历程的任何更早时间访问过的排名最前的页面。

1. 选择您想在哪里显示维度项：

   * [!UICONTROL **在历程画布中**]：将前 3 名节点添加到历程中此节点之前的画布中。 每个节点都作为画布上的单独分支连接到您选择的节点。
...
* 在画布上，选择要合并的节点，右键单击其中一个选定的节点，然后选择&#x200B;[!UICONTROL **合并节点**]。
...
您可以将组合节点中包含的组件拆分为它们自己的单独节点：

1. 右键单击要拆分的节点，然后选择&#x200B;[!UICONTROL **将组件拆分为节点**]。
...
当Journey Optimizer旅程包含同一两个节点之间的多个路径（例如，导致同一下一步的不同条件）时，每个路径都显示为单独的箭头。
...
1. 在历程画布可视化中，执行以下任一操作：
   ...
   * 右键单击要为其创建受众的节点的流失（显示流失时）。

   * 选择您要为其创建受众的一个或多个节点，然后右键单击其中一个选定的节点。

   * 在2个要为其创建受众的节点之间选择一个或多个箭头，然后右键单击其中一个选定的箭头。
...
1. 选择&#x200B;[!UICONTROL **从节点创建区段**]、[!UICONTROL **从箭头创建区段**]&#x200B;或&#x200B;[!UICONTROL **从流失创建区段**]。
   ...
   现在会显示区段生成器。 在&#x200B;[!UICONTROL **定义**]&#x200B;部分中，区段定义是根据您选择的节点、箭头或流失及其在历程中的上下文创建的。
...
您可以一次删除历程中的一个或多个节点。 如果您删除历程中位于 2 个节点之间且与其相连的节点，剩余的 2 个节点就会直接连接。
...
您可以更改历程中节点之间的箭头曲率和样式。

#### 更改箭头的曲率

要更改历程画布中箭头的曲率，请执行以下操作：

1. 在历程画布可视化中，在两个节点之间拖动箭头。

要将箭头重置回其默认位置，请执行以下操作：

1. 右键单击要重置的箭头。

1. 选择&#x200B;[!UICONTROL **重置箭头位置**]。

   如果同一两个节点之间存在多个箭头，则这些节点之间的所有箭头都将重置为其默认位置。

#### 更改箭头的样式

要更改历程画布中的箭头样式：

1. 在历程画布可视化中，右键单击要更改样式的两个节点之间的箭头。

1. 选择&#x200B;[!UICONTROL **更改箭头样式**]，然后选择以下样式之一：
