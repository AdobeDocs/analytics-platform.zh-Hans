...
1. （可选）从选区创建筛选器。
   ...
   选择单元格（连续或非连续），然后右键单击> **[!UICONTROL 从选择创建过滤器]**。

1. 在[过滤器生成器](/help/components/filters/manage-filters.md)中，进一步编辑区段，然后单击&#x200B;**[!UICONTROL 保存]**。

   保存的过滤器可用于[!UICONTROL Analysis Workspace]的[!UICONTROL 过滤器]面板。
1. 命名并保存同类群组项目。
