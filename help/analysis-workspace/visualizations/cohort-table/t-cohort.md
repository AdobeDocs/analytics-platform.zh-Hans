...
1. （可选）根据选定的内容创建过滤器。
   ...
   选择单元格（连续或不连续），然后右键单击鼠标> **[!UICONTROL 从选定范围中创建过滤器]**。

1. 在[过滤器生成器](/help/components/filters/manage-filters.md)中，进一步编辑过滤器，然后单击&#x200B;**[!UICONTROL 保存]**。

   保存的过滤器可用于[!UICONTROL Analysis Workspace]的[!UICONTROL Filter]面板中。
1. 命名并保存同类群组项目。
