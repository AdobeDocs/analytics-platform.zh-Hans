...
[!UICONTROL Analysis Workspace] 提供丰富的[键盘快捷键](/help/analysis-workspace/build-workspace-project/fa-shortcut-keys.md)（热键），以实现更顺畅的工作流。
...
在构建组件、可视化效果或面板时，进行保存时会验证必填字段。 如果必填字段未通过验证，则会以红色轮廓标出，并带有错误图标。 书面说明解释了需要修复的内容。
