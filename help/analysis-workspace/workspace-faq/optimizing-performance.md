...
| Internet 带宽 | 仅适用于 Google Chrome。您的浏览器对您所在位置的带宽的评估。评估准则为 2.0MB/秒。 | 您的本地网络连接将影响这一因素。 | 验证您的本地网络连接。 |
...
| 自由格式表的单元格数量 | 项目中自由格式表单元格的总量，计算方法是所有表的行数乘以列数。不包括隐藏的数据源。该准则为 4,000。 | 应当将表中的列数减少至仅包含最相关的数据点。通过调整显示的行数、应用表过滤器或应用过滤器，减少表中的行数。 |
