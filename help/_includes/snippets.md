...
| 选项 | 功能 |
|---------|----------|
| **[!UICONTROL 建议]** | 根据组件的推荐对每个类型（维度、量度、区段和日期范围）的组件进行排序。 您或组织中的其他人最常使用且最近使用的组件在每个列表中都显示在较高的位置。 |
| **[!UICONTROL 上次修改时间]** | 根据组件的上次修改日期，对每种类型（维度、量度、区段和日期范围）的组件进行排序。 最近修改的组件会在每个列表中突出显示。 |
| **[!UICONTROL 按字母顺序]** | 按升序字母顺序对每个类型（维度、量度、区段和日期范围）的组件排序。 |
| **[!UICONTROL 分类]** | 根据组件的类别对每个类型（维度、量度、区段和日期范围）的组件进行排序。 例如，策划的数据视图组件与非策划的数据视图组件。 |
...
| 标记 | 描述 |
|---|---|
| ![标记](/help/assets/filter-tag.png){width="300"} | 您可以使用&#x200B;**[!UICONTROL 标记]**&#x200B;部分按标记进行过滤。 <ul><li>您可以![搜索](/help/assets/icons/Search.svg) *搜索标记*&#x200B;以搜索可用于过滤的标记。</li><li>您可以选择多个标记。可用的标记取决于在过滤器面板中对其他部分的选择。</li><li>这些数字表明：<ul><li>**(1)**：所选标记的数量（如果选择了一个或多个标记）。</li><li>**2︎⃣**：可用于当前筛选器生成的项目的标记数。</li><li>⃣7︎：与特定标记关联的项目数。</li></ul></li></ul> |
...
| 数据视图 | 描述 |
|---|---|
| ![数据视图](/help/assets/filter-dataview.png){width="300"} | 您可以使用&#x200B;**[!UICONTROL 数据视图]**&#x200B;部分来过滤数据视图。 <ul><li>您可以![搜索](/help/assets/icons/Search.svg) *搜索数据视图*&#x200B;以搜索可用于筛选的数据视图。</li><li>您可以选择多个数据视图。可用的数据视图取决于在过滤器面板中对其他部分的选择。</li><li>这些数字表明：<ul><li>**(2)**：所选数据视图的数量（如果选择了一个或多个数据视图）。</li><li>**3︎⃣**：对当前筛选器生成的项可用的数据视图数。</li><li>⃣4︎：与特定数据视图关联的项目数。</li></ul></li></ul> |

## 已启用状态筛选器部分 {#enabledstatusfiltersection}

| 启用状态 | 描述 |
|---|---|
| ![已启用状态](/help/assets/filter-enabledstatus.png){width="300"} | **[!UICONTROL 启用状态]**&#x200B;部分允许您筛选启用状态。 <ul><li>您可以选择多个状态。</li><li>这些数字表明：<ul><li>**(2)**：选定状态的数量（如果选择了一个或多个状态）。</li><li>**2︎⃣**：从当前筛选器生成的项目可用的状态数。</li><li>⃣1︎：与特定状态关联的项目数。</li></ul></li></ul> |

## 键入筛选器部分 {#typefiltersection}

| 类型 | 描述 |
|---|---|
| ![Type](/help/assets/filter-type.png){width="300"} | **[!UICONTROL 类型]**&#x200B;部分允许您按类型筛选。 <ul><li>您可以选择多种类型。</li><li>这些数字表明：<ul><li>**(2)**：所选类型的数量（如果选择了一个或多个类型）。</li><li>**1︎⃣**：当前筛选器生成的项可用的类型数。</li><li>⃣3︎：与特定类型关联的项目数。</li></ul></li></ul> |

## “所有者”筛选器部分 {#ownerfiltersection}

| 所有者 | 描述 |
|---|---|
| ![所有者](/help/assets/filter-owners.png){width="300"} | **[!UICONTROL 所有者]**&#x200B;部分允许您过滤所有者。 <ul><li>您可以![搜索](/help/assets/icons/Search.svg) *搜索所有者*&#x200B;以搜索可用于筛选的所有者。</li><li>您可以选择多个所有者。可用的所有者取决于在过滤器面板中对其他部分的选择。</li><li>这些数字表明：<ul><li>**(2)**：所选所有者的数量（如果选择了一个或多个所有者）。</li><li>**3︎⃣**：从当前筛选器生成的项目可用的所有者数。</li><li>⃣4︎：与特定所有者关联的项目数。</li></ul></li></ul> |

## 其他筛选器筛选器部分 {#otherfiltersfiltersection}

| 其他过滤器 | 描述 |
|---|---|
| ![其他过滤器](/help/assets/filter-other.png){width="300"} | **[!UICONTROL 其他过滤器]**&#x200B;部分允许您过滤其他预定义过滤器。<ul><li>您可以选择以下一个或多个选项：<ul><li> **[!UICONTROL 显示所有]**</li><li>**[!UICONTROL 与我共享]**</li><li>**[!UICONTROL 我的]**</li><li>**[!UICONTROL 已批准]**</li><li>**[!UICONTROL 收藏夹]**</li></ul> 您可以选择的内容取决于您的角色和权限。</li><li>您可以选择多个其他过滤器。可用的其他过滤器取决于在过滤器面板中对其他部分的选择。</li><li>这些数字表明：<ul><li>**(1)**：已选择的其他筛选器的数量（如果已选择一个或多个其他筛选器）。</li><li>**5︎⃣**：当前筛选器生成的项目可用的其他筛选器数。</li><li>⃣4︎：与特定其他过滤器关联的项目数。</li></ul></li></ul> |

## 日期范围过滤器部分  {#daterangefiltersection}

| 应用的日期范围 | 描述 |
|---|---|
| ![日期范围](/help/assets/filter-daterange.png){width="300"} | 通过应用的日期范围部分，可筛选适用于项的日期范围。<ol><li>选择日期范围。</li><li>在日历弹出窗口中，定义日期范围，或选择一个可用预设。<br>或者，您也可以直接在“筛选器”面板的“日期范围”部分指定日期范围。</li></ol><ul><li>这些数字表明：<ul><li>**(1)**：从默认预设修改的修改日期范围数。</li><li>**5︎⃣**：当前筛选器生成的项可用的日期范围数。</li></ul> |
...
| ![反向 J](/help/assets/icons/AttributeInverseJ.svg) | 反向 J | 将 60% 的点数分给首次接触点，20% 的点数分给最后接触点，并将剩余 20% 的点数分给这两次接触点之间的任意接触点。对于具有单一接触点的转化，它将分得 100% 的点数。对于具有两个接触点的转化，将75%的点数分给首次交互，将25%的点数分给最后一次交互。 与J形类似，此归因模型更支持首次交互和最后交互，但更支持首次交互。 |
...
* **60天**：自转化发生后最多可回顾60天。
* **90天**：自转化发生后最多可回顾90天。
* **13个月** [!BADGE B2B edition]{type=Informative}：自转化发生后可回顾13个月。
...
* **全局帐户（报表窗口）** [!BADGE B2B edition]{type=Informative}：回顾到当前日期范围当月第一个月的所有全局帐户。 例如，如果报表日期范围为9月15日到9月30日，则全局帐户回顾日期范围将为9月1日到9月30日。 如果使用此回顾窗口，则偶尔可以看到维度项目归因到报告窗口之外的日期。
* **帐户（报表窗口）** [!BADGE B2B edition]{type=Informative}：回顾到当前日期范围当月第一个月的所有帐户。 例如，如果报表日期范围为9月15日到9月30日，则帐户回顾日期范围将为9月1日到9月30日。 如果使用此回顾窗口，则偶尔可以看到维度项目归因到报告窗口之外的日期。
* **Opportunity （报告窗口）** [!BADGE B2B edition]{type=Informative}：回顾到当前日期范围当月第一个月的所有机会。 例如，如果报表日期范围为9月15日到9月30日，则机会回顾日期范围将为9月1日到9月30日。 如果使用此回顾窗口，则偶尔可以看到维度项目归因到报告窗口之外的日期。
* **购买群（报告窗口）** [!BADGE B2B edition]{type=Informative}：回顾到当前日期范围当月第一个月的所有购买群。 例如，如果报表日期范围为9月15日到9月30日，则购买组回顾日期范围将为9月1日到9月30日。 如果使用此回顾窗口，则偶尔可以看到维度项目归因到报告窗口之外的日期。
...
| 节点或接触点中的&#x200B;**组件支持** | 量度、维度项目、区段和日期范围。 | 量度、维度项目、区段和日期范围。 | 仅限维度项目（开始接触点和结束接触点除外） |
| **比较细分** | 否 | 是<p>对同一个报告中的两个不同区段执行逐项比较。</p> | 否 |
| **拖放组件交互** | 是 | 是 | 否 |
