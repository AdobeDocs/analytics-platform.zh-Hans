...
| [!UICONTROL 日期] | 发生给定事件的那一个月中的第几天。 第一个维度项是日期范围内当月的第一天，最后一个维度项是日期范围内当月的最后一天。 |
| 事件深度 | 将顺序数值（1、2、3等）分配给会话中的每个事件交互。 通过此维度，您可以启用详细跟踪和分析，以跟踪和分析在为数据视图](/help/data-views/session-settings.md#session-settings)定义的[受限体验会话中，特定事件在顺序的用户交互流中发生的位置。 您可以跟踪事件在有界会话中从开始到结束的进度。 例如：访客登陆您的主页（事件1，会话开始），使用搜索功能（事件2），查看产品详细信息页面（事件3），添加到购物车（事件4），进入结账（事件5），并完成购买（事件6，会话结束）。 现在，您可以在区段定义中使用[!UICONTROL 事件深度]来根据交互深度划分数据。 |
