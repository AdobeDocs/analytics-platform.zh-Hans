...
| **不区分大小写的多值变量** | 对于不区分大小写的多值变量，存储在 `mvvar1` 至 `mvvar3` 中的值将不再自动变为小写。相反，通过 Analytics Source Connector 传递到 Adobe Experience Platform 和 CJA 的数据将反映从页面传入的原有大小写形式。源连接器和CJA列 `_experience.analytics.customDimensions.lists.list1.list[]` - `_experience.analytics.customDimensions.lists.list3.list[]` 会受此更改的影响。 | 不适用 | 2022 年 10 月 24 日 |
...
| **HIPAA 准备就绪** | Adobe现在支持在Customer Journey Analytics和其他基于Experience Platform的应用程序中接收、使用、维护或传输受保护的健康信息，以便 [!DNL Healthcare Shield] 仅限客户。 [!DNL Healthcare Shield] 仅适用于美国的受保实体或业务伙伴的医疗保健客户。[了解详情](https://www.adobe.com/trust/compliance/hipaa-ready.html) | 不适用 | 2022 年 11 月 7 日 |
...
| **改进了 IP 到地理位置的映射** | 2023 年 1 月 5 日 | Adobe 的 IP 查找供应商 Digital Element 正在升级到经过改进的新数据集 (NetAcuity Pulse) 以进行 IP 到地理位置映射。Adobe Analytics 已推迟到 **2023 年 1 月 11 日**&#x200B;采用此新数据集。新的数据库将比以前的版本更准确。 当采用新数据库时，一些 IP 到地理位置的映射将发生变化/改进。<p> 通过 [!UICONTROL Analytics Source Connector] 提供的 CJA 数据也将自动利用新映射。 |
...
| [!UICONTROL PersonID] 和 [!UICONTROL PersonID 命名空间]作为维度 | 公开 `personID` (或 `customerID`，或用于合并连接中数据集的任何ID)作为数据视图中的维度使用。 此增强功能使您能够从连接拉入 `personID`，更轻松地将其作为维度包含在数据视图中。[了解详情](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-dataviews/component-reference.html?lang=zh-Hans#optional-standard-components) | 2022 年 1 月 19 日 |
