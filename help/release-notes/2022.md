...
| **不区分大小写的多值变量** | 对于不区分大小写的多值变量，存储在 `mvvar1` 至 `mvvar3` 中的值将不再自动变为小写。相反，通过Analytics Source Connector传递到Adobe Experience Platform和Customer Journey Analytics的数据将反映从页面传入的原有大小写形式。 “源连接器”和“Customer Journey Analytics”列 `_experience.analytics.customDimensions.lists.list1.list[]` - `_experience.analytics.customDimensions.lists.list3.list[]` 将受到此更改的影响。 | 不适用 | 2022 年 10 月 24 日 |
| **Customer Journey Analytics审核日志** | Customer Journey Analytics允许您通过“审核日志”的形式审核各种服务和功能的用户活动。 这些日志形成审核记录，可以帮助解决问题，并帮助您的企业有效遵守公司数据管理政策和监管要求，例如“健康保险流通与责任法案”(HIPAA)。这些日志以前只能通过审核日志 API 获得。[了解详情](/help/privacy/audit-log.md) | 不适用 | 2022 年 10 月 26 日 |
| **HIPAA 准备就绪** | Adobe现在支持接收、使用、维护或发送Customer Journey Analytics和其他基于Experience Platform的应用程序中的受保护的健康信息， [!DNL Healthcare Shield] 仅限客户。 [!DNL Healthcare Shield] 仅适用于美国的受保实体或业务伙伴的医疗保健客户。 [了解详情](https://www.adobe.com/trust/compliance/hipaa-ready.html) | 不适用 | 2022 年 11 月 7 日 |
...
| **“试验”面板** | 这个新的“工作区”面板允许Customer Journey Analytics用户从任何来源评估任何A/B试验的提升和置信度：在线、离线、Adobe解决方案、Adobe Journey Optimizer甚至BYO数据。 [了解详情](/help/analysis-workspace/c-panels/experimentation.md) | 2022 年 10 月 5 日 |
...
| **Customer Journey Analytics中的日期字段支持** | 允许Customer Journey Analytics报告日期和日期时间字段。 [了解详情](/help/use-cases/data-views/data-views-usecases.md) | 2022 年 10 月 5 日 |
...
| **Customer Journey Analytics的“试验”面板** | 这个新的“工作区”面板允许Customer Journey Analytics用户从任何来源评估任何A/B试验的提升和置信度：在线、离线、Adobe解决方案、Adobe Journey Optimizer甚至BYO（自带）数据。 [了解详情](/help/analysis-workspace/c-panels/experimentation.md) | [限量发行](/help/release-notes/releases.md)从 2022 年 9 月 14 日开始 |
...
| **Customer Journey Analytics支持“数据管理”标签和策略** | 自动化Customer Journey Analytics与Adobe Experience Platform隐私标签和策略之间的集成。 在Platform使用的数据集上创建的数据标签显示在Customer Journey Analytics数据视图中，用以阻止或警告从敏感字段创建量度和/或维度的用户。 此外，从Customer Journey Analytics导出数据时(通过Workspace或Report Builder报表、导出、API等)， 将添加其他警告或标签，以通知用户报告包含需要以特定方式处理的敏感信息。 [了解详情](/help/data-views/data-governance.md) | 2022 年 9 月 14 日 |
...
| Analytics Source Connector的数据准备 | 此 [Analytics源连接器](https://experienceleague.adobe.com/docs/experience-platform/sources/ui-tutorials/create/adobe-applications/analytics.html?lang=zh-Hans) 现已与 [数据准备](https://experienceleague.adobe.com/docs/experience-platform/data-prep/home.html?lang=zh-Hans) 功能由Adobe Experience Platform提供。 Adobe Real-time Customer Data Platform (RTCDP)、Customer Journey Analytics和Adobe Journey Optimizer客户现在可以通过其他字段组扩展Analytics字段组。 他们还可以利用100多个数据准备操作员，在将数据摄取到Adobe Experience Platform期间扩充Analytics数据。 RTCDP客户现在可以为配置文件启用多个支持数据准备的报告包。<p>现在，通过Analytics源连接器摄取多个报告包的Customer Journey Analytics客户可以在报告包之间取消对列差异的冲突。 例如，如果“搜索词”存储在 `eVar1` 在一个报告包中 `eVar2` 在另一个报告包中，使用数据准备，您可以通过合并两个eVar中的值的新列来扩展Analytics字段组。 | 2022 年 4 月 27 日 |
...
| [!UICONTROL PersonID] 和 [!UICONTROL PersonID 命名空间]作为维度 | 公开 `personID` (或 `customerID`，或者您用来在连接中合并数据集的任何一个ID)作为数据视图中的维度。 此增强功能使您能够从连接拉入 `personID`，更轻松地将其作为维度包含在数据视图中。[了解详情](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-dataviews/component-reference.html?lang=zh-Hans#optional-standard-components) | 2022 年 1 月 19 日 |
