...
| 资产转移 | 允许您[将组件（如项目、区段和计算量度）的所有权](https://experienceleague.adobe.com/zh-hans/docs/analytics-platform/using/tools/asset-transfer/transfer-assets)转移给其他用户，以确保连续性和适当的访问权限。 |
...
| Workspace 和组件 | 刷新了有关Analysis Workspace项目（项目、可视化和面板）和组件(注释、维度、（计算）指标、区段、日期范围、警报、计划项目和受众)的文档。 |
| 引导式分析 | 更新了文档，其中包含来自 Analysis Workspace 中的[引导式分析](/help/guided-analysis/overview.md)。 |
| 更新的受众文档 | 现在，在Analysis Workspace中通过可视化图表[创建受众](https://experienceleague.adobe.com/zh-hans/docs/analytics-platform/using/cja-components/audiences/publish?lang=en)时，将面板区段和列区段作为附加条件包括在内。 |
| **2024 年 9 月** | |
