...
| **更新了 XDM 字段，用于将流媒体数据收集到 Adobe Experience Platform** | 在将流媒体数据收集到Adobe Experience Platform时，不应再使用`media.mediaTimed`字段组。 客户应该改迁移到`mediaReporting`字段组。<p>在三个月的过渡期内，`media.mediaTimed` 字段的数据摄取将继续进行。但是，从 2025 年 7 月末起，`the media.mediaTimed` 字段将被完全弃用，并且不再显示在 Adobe Experience Platform Schema UI 中，只能使用 `mediaReporting` 字段发送数据。<p>在2025年4月22日之前实施了Analytics Source Connector以将流媒体数据收集到Platform的客户必须迁移其现有配置以使用新的字段组。 此迁移必须在 2025 年 7 月底之前完成。请联系您的 Adobe Consulting 服务或账户团队以获取迁移支持。在 2025 年 4 月 22 日之后实施 Analytics 源连接器的客户无需采取任何行动。 |  | 2025 年 4 月 22 日 |
| **术语更改：将“过滤器”改为“区段”** | 此前，Adobe Customer Journey Analytics 将区段称为“过滤器”。该术语现在改成与 Adobe Analytics 保持一致。“过滤器”现在改称为“区段”。（显然，搜索过滤器仍然称为“过滤器”。）UI 已更新，文档正在更新中。 |  | 2025 年 4 月 16 日 |
| **拼接：从 XDM IdentityMap 检索持久和临时 ID** | 此功能支持在拼接过程中使用 XDM IdentityMap 中存储的身份标识。IdentityMap 可用于在基于字段的拼接中使用持久 ID 或临时 ID，也可用于在基于图的拼接中使用持久 ID。您可以使用 identityMap 中的某个特定命名空间或主要身份标识。（文档链接见下文） |  | 2025 年 4 月 25 日 |
| **跨数据视图共享的量度和维度** | 允许您在多个数据视图中应用维度和量度设置。对某个共享维度或量度所做的更改将应用于所有适用的数据视图中该维度或量度的所有实例。此界面允许 Customer Journey Analytics 管理员在使用许多数据视图时更轻松地管理组件。（文档链接见下文） |  | 2025 年 4 月 30 日 |
