...
# 当前 Customer Journey Analytics 发行说明（2025 年 9 月）
...
| **支持临时的基于模型的架构** | [临时](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/xdm/tutorials/ad-hoc)的基于模型的架构用于 Experience Platform 的数据摄取和 Data Mirror 工作流。 |  | 2025 年 9 月 23 日（原计划于 2025 年 8 月 28 日发布） |
| **实时报告** | Customer Journey Analytics 中的实时报告功能可以实时显示并更新 Analysis Workspace 的一个或多个面板中的数据和可视化图表。<br/><br/>（文档链接见下文。） | 2025 年 9 月 18 日（原计划于 2025 年 8 月 15 日发布） | 2025年10月8日 |
| **支持 Data Mirror** | 通过对 Experience Platform 中的特定源连接器提供基于模型的架构以及变更数据捕获 (CDC) 功能的支持，Customer Journey Analytics 将能够支持 [!DNL Snowflake]、[!DNL Azure Databrick] 和 [!DNL Google BigQuery] 等数据仓库解决方案的 Data Mirror 功能。<p>要访问 Beta 版本，请联系您的 Adobe 帐户团队。</p><p>（文档链接见下文。）</p> | Beta 版，于 2025 年 9 月 24 日发布 | 待定 |
| **流媒体服务：支持计划数据** | 您现在可以更轻松、更准确地跟踪收视率，上传过去实时流媒体内容的计划数据。<p>以下是支持计划数据上传的直播内容示例：</p><ul><li>FAST（免费广告支持电视）平台</li><li>本地流</li><li>直播体育赛事</li></ul><p>上传计划数据允许您跟踪在上传文件中指定的时间内运行的各个节目的观看人数数据。您甚至可以收集特定主题或项目群区段的收视率数据。</p><p>无论您如何实现流媒体收集，这些功能都是可用的。</p><p>以前，在分析实时内容时，很难将给定会话准确地绑定到特定项目，并且无法将给定会话绑定到单个主题或项目群区段。</p><p>（文档链接见下文。）<!--For more information, see [Upload schedule data to track live content](https://experienceleague.adobe.com/zh-hans/docs/media-analytics/using/media-use-cases/track-schedule-data)--></p> |  | 2025年10月29日 |
