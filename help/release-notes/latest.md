...
| ----------- | ---------- | ------- | ---- |
| **摘要级别数据源** | 允许您引入没有人员 ID 的时间序列数据。该时间序列数据可用于支持各种用例，例如：<ul><li>将高级性能指标作为事件级数据的一部分或在其旁边呈现。这可以包括诸如日期和单个指标值之类的简单内容，也可以包括多个维度和指标，例如广告展示、电子邮件打开、广告支出、销售成本等。</li><li>以每小时或每天上传一次目标或目标，然后根据事件级别的量度定位这些目标或目标。 这有助于直观显示指标相对于组织目标或目标的趋势。</li></ul><p>有关详细信息，请参阅[摘要数据](/help/data-views/summary-data.md)</p> |  | 2024年8月13日 |
