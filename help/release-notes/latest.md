...
| **摘要级别数据源** | 允许您引入没有人员 ID 的时间序列数据。该时间序列数据可用于支持各种用例，例如：<ul><li>将高级性能指标作为事件级数据的一部分或在其旁边呈现。这可以包括诸如日期和单个指标值之类的简单内容，也可以包括多个维度和指标，例如广告展示、电子邮件打开、广告支出、销售成本等。</li><li>按日、周、月或季度上传目标，并根据事件级指标定位这些目标，以帮助直观地了解指标相对于组织目标的趋势。</li></ul><p>（已更新的文档链接见下文）</p> |  | 2024 年 8 月 11 日 |
