...
| **导出完整表格时禁用清单文件** | 将允许您禁用在从Analysis Workspace导出完整表时默认包含的清单文件。 [了解详情](/help/analysis-workspace/export/export-cloud.md) |  | 2025年5月20日 |
...
| 双精度类型维度的&#x200B;**Dimension格式默认为2** | 对于具有双重数据类型的架构，维度格式现在默认为2位小数。 您可以将此数字更改为0到5位小数。<p>以前，格式默认为0位小数。</p><p>这意味着，如果在Analysis Workspace报表中使用双类型维度，则默认情况下不显示小数位。 这些相同的报表现在将显示2位小数。</p><p>有关如何更新双重类型维显示的小数位数的详细信息，请参阅[格式组件设置](/help/data-views/component-settings/format.md)。</p> | | 2025 年 5 月 29 日 |
| **Analysis Workspace 左侧面板不再在鼠标悬停时打开和关闭** | Analysis Workspace 中的左侧面板用于将组件、面板和可视化图表等元素添加到您的项目中。将鼠标悬停在最左侧的一个图标上以临时打开左侧面板，这个选项不再可用。现在，只需单击其中一个图标即可保持面板打开，然后单击相同的图标可将其关闭。 |  | 2025年6月2日 <p>（原计划于2025年5月29日发布）</p> |
