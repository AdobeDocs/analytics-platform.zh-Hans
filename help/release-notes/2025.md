...
| **将 Adobe Analytics 项目和其中包括的任何组件迁移到 Customer Journey Analytics 的 API** | 现在可以使用 API 将您的 Adobe Analytics 项目及其中包含的组件迁移到 Customer Journey Analytics。以前，项目和组件迁移只能通过用户界面进行。[了解详情](https://adobedocs.github.io/analytics-2.0-apis/?urls.primaryName=CJA%20Migration%20APIs)。从下拉菜单中选择&#x200B;**CJA迁移API**。 |  | 2025 年 1 月 15 日 |
