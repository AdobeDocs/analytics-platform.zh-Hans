...
subfeature_v2: id: aff2ef09-fc60-4018-9197-e2befd623064id: b1f5d324-a668-4e51-a59b-6fc0862d7310id: bc7a5a86-1a70-451f-985c-037b65f091d1id: c38ed341-fab2-46df-9d72-88d8166edebbid: d1d3b429-e0a8-4e2f-af0a-a48d23e366b7id: d3c978ee-1ff0-4475-968a-721e2dd99ef1id: df7fb1db-aa1b-4314-98ac-59dbfcc3044f
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
topic_v2: id: aa2f3246-cb95-4b30-8899-fdf7d73550ccid: cc72dcf1-72e1-48cc-b434-e7c27d62d67cid: cdd65e7e-8839-44a2-bc21-0e03623b5dd1id: d00e9f03-e50b-4162-b143-0c0817c937c2id: e1e0219c-f879-479f-8427-888ed2a6e9c2
...
| ----------- | ---------- | ----- |
| Customer Journey Analytics 文档 | 2020 年 11 月 11 日 | Customer Journey Analytics 文档已迁移至 Experience League。 在 11 月份，所有的文章、视频、发行说明和教程都已从其当前位置 (`experienceleague.adobe.com`) 迁移至 `experienceleague.adobe.com`。 此次迁移可确保从一个位置提供所有的学习、自助、赋能和社区内容。 所有链接都已重定向到 Experience League。 |
| 折线图可视化图表：移动平均趋势线选项 | 2020 年 10 月 8 日 | 移动平均已添加到折线图可视化图表的趋势线设置。 移动平均值计算指定前期的平均值并将其用作趋势线数据点，然后移动到下一个期间。 [了解详情](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/visualizations/line.html?lang=zh-hans) |
| 删除回填限制 | 2020 年 10 月 19 日 | 为了提供更好的 Customer Journey Analytics 体验，我们移除了回填（历史数据导入）限制。 以前，您自己最多可以回填 25 亿行，超出这个限制则需工程团队介入。 现在，您自己即可不受任何限制地回填数据。 [了解详情](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-connections/create-connection.html#enable-connection) |
...
| 跨渠道分析 | 2021 年 1 月 11 日 | 跨渠道分析功能允许客户为Adobe Experience Platform数据湖中的Adobe Analytics（或其他）事件数据集重新生成键值，即，从一个ID命名空间的键值重新生成另一个ID命名空间的键值。 通常，这意味着需要将事件数据集的键从基于 Cookie 的 ID 重新设为基于个人的 ID。 通过这种方式，重新生成键值的数据集可以同 Customer Journey Analytics 连接中其他基于人员的数据相结合，从而在 Analysis Workspace 中实现跨设备/跨渠道分析。 [了解详情](../stitching/overview.md) |

## 2020 年 10 月

| 功能 | [正式发布](https://experienceleague.adobe.com/docs/analytics/landing/an-releases.html?lang=zh-Hans) — 目标日期 | 描述 |
| ----------- | ---------- | ----- |
| Customer Journey Analytics 文档 | 2020 年 10 月 | Customer Journey Analytics 文档正在迁移至 Experience League。 在 10 月份，所有的文章、视频、发行说明和教程都将从其当前位置 (`experienceleague.adobe.com`) 迁移至 `experienceleague.adobe.com`。 此次迁移可确保从一个位置提供所有的学习、自助、赋能和社区内容。 当这项变更发生时，您无需执行任何操作，因为所有链接都将重定向到 Experience League。 我们会在切换开始时更新发行说明。 |
| [!UICONTROL 工作区]：下载适合单个维度的 50,000 个项目 | 2020 年 9 月 17 日 | 您将能够在自由格式表中下载适合单个维度的50,000个项目，并应用区段。 这允许您访问 Analysis Workspace 以外的 400 多行数据。 [了解详情](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-workspace/curate-share/download-send.html) |
...
| 功能 | [正式发布](https://experienceleague.adobe.com/docs/analytics/landing/an-releases.html?lang=zh-Hans) — 目标日期 | 描述 |
| ----------- | ---------- | ----- |
| 更改 Customer Journey Analytics 的权限 | 2020 年 9 月 9 日 | Customer Journey Analytics 将不再把所有用户视为管理员。 只有在 [Adobe Admin Console](https://experienceleague.adobe.com/docs/core-services/interface/manage-users-and-products/admin-getting-started.html) 中被指派为产品管理员的用户，才能执行以下操作：<ul><li>创建/更新/删除[!UICONTROL 连接]或[!UICONTROL 数据视图]</li><li>更新/删除其他用户创建的项目、区段或计算量度</li><li>将工作区项目共享给所有用户</li></ul>[了解详情](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-overview/cja-overview.html#admin-access-permissions) |
| 对[!UICONTROL 异常检测]的支持 | 2020 年 9 月 10 日 | [!UICONTROL 异常检测]允许您识别哪些统计波动重要，哪些不重要。 [!UICONTROL Customer Journey Analytics]现在支持此功能。 |
...
| 功能 | [正式发布](https://experienceleague.adobe.com/docs/analytics/landing/an-releases.html?lang=zh-Hans) — 目标日期 | 描述 |
| ----------- | ---------- | ----- |
| 工作区：折线图可视化图表增强功能 | 2020 年 9 月 17 日 | <ul><li>可显示或隐藏任何折线图可视化图表的 x 轴和 y 轴。 如果想让您的折线图可视化图表更加紧凑，此功能会特别有用。</li><li>您可以在任何折线图可视化图表上叠加最小值和最大值标签，以便快速突出显示量度中的峰值和谷值。</li><li>您可以在任何折线图可视化图表上叠加不同的回归趋势线，以便更轻松地查看数据的趋势。 相关选项包括线性、对数、指数、幂和二次函数。</li></ul>[了解详情](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/visualizations/line.html?lang=zh-hans) |

## 2020 年 6 月

| 功能 | [正式发布](https://experienceleague.adobe.com/docs/analytics/landing/an-releases.html?lang=zh-Hans) — 目标日期 | 描述 |
| -----------| ---------- |-----|
| 支持对象数组 | 2020 年 6 月 18 日 | Customer Journey Analytics 客户现在可以针对其 Adobe Experience Platform 数据集架构内对象数组中显示的维度和量度生成报告。 [了解详情](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-usecases/complex-data/object-arrays.html?lang=zh-Hans) |
...
| 仅查看[!UICONTROL 工作区]项目 | 2020 年 6 月 18 日 | 可以将[!UICONTROL 工作区]项目仅以&#x200B;_[!UICONTROL 可以查看]_&#x200B;的形式共享给用户。 当具有“查看”角色的收件人打开共享项目时，他们将获得比较受限的项目体验，因为项目无左边栏而且可进行的交互也有限。 [了解详情](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-workspace/curate-share/view-only-projects.html) |
| 能够共同编辑[!UICONTROL 工作区]项目 | 2020 年 6 月 18 日 | 添加到&#x200B;_[!UICONTROL 可以编辑]_&#x200B;角色的收件人可以保存已共享给他们的项目。 [了解详情](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-workspace/curate-share/share-projects.html) |
| [!UICONTROL 工作区]中的“快速洞察”面板 | 2020 年 6 月 25 日 | 快速洞察可为 [!UICONTROL Analysis Workspace] 的非分析师和新用户了解如何快速轻松地回答业务问题提供指导。 [了解详情](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-workspace/panels/quickinsight.html) |
...
| 功能 | [正式发布](https://experienceleague.adobe.com/docs/analytics/landing/an-releases.html?lang=zh-Hans) — 目标日期 | 描述 |
| -----------| --------- | ---------- |
| [!UICONTROL Workspace] 中的 [!UICONTROL Quick Insights] 面板 | 2020 年 6 月 25 日 | [!UICONTROL 快速洞察]可为 [!UICONTROL Analysis Workspace] 的非分析师和新用户了解如何快速轻松地回答业务问题提供指导。 [了解详情](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/panels/quickinsight.html) |
