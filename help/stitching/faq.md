...
| 拼接数据集 | Id | 时间戳 | 永久 ID | 永久命名空间 | 人员 ID | 人员命名空间 | 拼接 ID | 拼接命名空间 |
|---|---|---|---|---|---|---|---|---|
...
* 如果人员ID字段不为空，则Customer Journey Analytics使用人员ID中的值作为拼接ID。
* 如果人员ID字段为空，则Customer Journey Analytics还会将拼合的ID留空。 在这种情况下，事件的永久ID、人员ID和拼接ID全部为空。 使用拼接的数据集从任何Customer Journey Analytics连接中删除这些类型的事件，其中拼接的ID被选作人员ID。
...
| 事件 | 时间戳 | 永久 ID（Cookie ID） | 临时 ID | 拼接 ID（重播后） |
|---|---|---|---|---|
