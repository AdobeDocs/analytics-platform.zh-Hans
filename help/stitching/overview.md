...
- **重播拼接**：根据它所获知的唯一标识符（临时ID）“重播”数据。 在此阶段，来自先前未知设备（永久ID）的点击将被拼合（到临时ID）。 Adobe提供以下重播时间间隔：
   ...
   - **每月**：数据每月使用回顾窗口重播一次（请参阅[选项](#options)）。 此选项的优势在于允许未经身份验证的会话在更加宽松充裕的时间范围内进行身份验证。但是，只有到下个月重放时，才会重新处理不到一个月前的未拼合数据。
...
- **重播拼接**：根据身份图形中更新的身份“重播”数据。 在这个阶段，来自先前未知设备（永久ID）的点击会在身份图解析命名空间身份后拼合。 Adobe提供以下重播时间间隔：
   - **每日**：数据每天重播，回顾时间范围为24小时。 此选项的优势在于重播更频繁，但未经身份验证的访客必须在访问您网站的同一天进行身份验证。
   - **每周**：数据每周重播一次，回顾时间范围为[选项](#options)。 此选项的优势在于允许未经身份验证的会话在更加宽松充裕的时间范围内进行身份验证。但是，只有到下一次每周重放时，才会重新处理不到一周的未拼合数据。
   - **每两周**：数据每两周重播一次，回顾时间范围为[选项](#options)。 此选项的优势在于允许未经身份验证的会话在更加宽松充裕的时间范围内进行身份验证。但是，只有等到下一次两周重放时，才会重新处理两周内的未拼合数据。
   - **每月**：数据每月使用回顾窗口重播一次（请参阅[选项](#options)）。 此选项的优势在于允许未经身份验证的会话在更加宽松充裕的时间范围内进行身份验证。但是，只有到下个月重放时，才会重新处理不到一个月前的未拼合数据。
