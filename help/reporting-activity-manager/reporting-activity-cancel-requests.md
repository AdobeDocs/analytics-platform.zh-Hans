...
* 在取消请求时，您还可以选择限制给定时间期的后续请求。
  ...
  当您限制后续请求时，该操作将记录在[审核日志](/help/privacy/audit-log.md)中，其操作名称为EMBASE。
...
您可以取消占用大量报告容量的单个请求。 取消请求时，您可以选择在给定时间段内进一步限制请求。
...
1. （可选）要限制给定时间段的将来请求，请执行以下操作：
   ...
   1. 启用选项&#x200B;[!UICONTROL **限制后续请求**]。
      ...
      ![取消1个请求，其中显示已选择限制后续请求和取消消息。](assets/restrict-subsequent-requests.png)
...
您可以取消与一个或多个用户关联的所有请求。 在取消与某个用户相关联的请求时，您可以选择进一步限制该用户在给定时间段内的请求。
...
1. （可选）要限制给定时间段的将来请求，请执行以下操作：
   ...
   1. 启用选项&#x200B;[!UICONTROL **限制后续请求**]
      ...
      ![取消1个请求，该请求显示按所选用户限制后续请求。](assets/restrict-subsequent-requests-user.png)
...
您可以取消与一个或多个项目关联的所有请求。 在取消与项目关联的请求时，您可以选择进一步限制在给定时间段内与该项目关联的请求。
...
1. （可选）要限制给定时间段的将来请求，请执行以下操作：
   ...
   1. 启用选项&#x200B;[!UICONTROL **限制后续请求**]。
      ...
      ![取消1个请求，按项目显示限制后续请求](assets/restrict-subsequent-requests-project.png)
...
1. （可选）要限制给定时间段的将来请求，请执行以下操作：
   ...
   1. 启用选项&#x200B;[!UICONTROL **限制后续请求**]
      ...
      ![取消1个请求，该请求显示“按选择的应用程序限制后续请求”。](assets/restrict-subsequent-requests-application.png)
