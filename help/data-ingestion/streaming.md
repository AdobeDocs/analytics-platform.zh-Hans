---
title: 摄取和使用流数据
description: 说明如何在Customer Journey Analytics中摄取和使用流数据
...
本快速入门指南介绍了如何将流数据引入Adobe Experience Platform，然后在Customer Journey Analytics中使用该数据。

要实现此目的，您需要：

- **设置架构和数据集** 在Adobe Experience Platform中，定义要收集的数据的模型（模式）以及实际收集数据（数据集）的位置。

- **使用HTTP API源连接器** 以轻松将数据流入在Adobe Experience Platform中配置的数据集。
...
>[!NOTE]
>
>这是有关如何将流数据摄取到Adobe Experience Platform并在Customer Journey Analytics中使用的简化指南。  强烈建议在提及时研究附加信息。
...
要将此数据流式传输到我们创建的数据集，您需要为要发送到的该数据定义流端点。 要执行此操作，请定义HTTP API源连接器。

要创建HTTP API源连接器，请执行以下操作：

1. 在Experience PlatformUI中，选择 **[!UICONTROL 源]** 在 [!UICONTROL 连接] 中。

2. 选择 **[!UICONTROL 流]** 从列表 [!UICONTROL 类别].

3. 选择 **设置** 在 [!UICONTROL HTTP API] 拼贴。

   ![设置HTTP API磁贴](./assets/httpapi-tile.png)

4. 在 [!UICONTROL 身份验证] 步骤 [!UICONTROL 添加数据] 屏幕：

   输入HTTP API连接的名称和描述。

   选择 **[!UICONTROL 兼容XDM]** 为指示要流传输的数据与现有XDM架构兼容。

   选择 **[!UICONTROL 连接到源]**. 成功连接后，您将看到 [!UICONTROL 已连接].

   ![HTTP API身份验证](./assets/httpapi-authentication.png)

   选择 **[!UICONTROL 下一个]** 继续。

5. 在 [!UICONTROL 数据流详细信息] 步骤 [!UICONTROL 添加数据] 屏幕：

   选择 **[!UICONTROL 现有数据集]**，从数据集列表中选择数据集并命名 [!UICONTROL 数据流名称].

   ![数据流详细信息](./assets/httpapi-dataflowdetail.png)

   选择&#x200B;**[!UICONTROL 下一步]**。

6. 的 [!UICONTROL 审阅] 步骤 [!UICONTROL 添加数据] 屏幕提供了HTTP API连接的概述。

   ![数据流详细信息](./assets/httpapi-review.png)

   选择 **[!UICONTROL 完成]**.

7. 您将看到HTTP API流端点的最终定义。

   ![HTTP API流端点](./assets/httpapi-streamingendpoint.png)

您可以复制流端点URL并使用它配置忠诚度应用程序，以将数据流式传输到Adobe Experience Platform忠诚度数据集。

请参阅 [使用UI创建HTTP API流连接](https://experienceleague.adobe.com/docs/experience-platform/sources/ui-tutorials/create/streaming/http.html?lang=en) 有关更全面的教程，请参阅如何使用身份验证、如何在传入数据与XDM架构不兼容时映射数据，以及如何在设置流连接器时创建新数据集。
...
>[!SUCCESS]
>
>您已完成所有步骤。 首先，定义要收集的忠诚度数据（架构）以及将其存储在Adobe Experience Platform中的位置（数据集），然后配置了一个HTTP API源连接器，以将忠诚度数据直接流到数据集中。 通过数据视图定义，您可以指定要使用的维度和量度，最后创建了您的第一个项目，并对数据进行可视化和分析。
