...
| **系统强制的护栏（硬限制）** | Customer Journey AnalyticsUI或API强制实施系统强制的护栏。 这些限制不得超过，因为UI和API将阻止您这样做或您会返回错误。 |
...
| 请勿重试超时 | 600 | 系统强制的护栏 | 临时SQL查询超时之前的最大秒数。 否则，报告引擎报告请求返回结果所花费的时间太长之前的最大秒数，不应重试，因为后台进程中的问题导致请求永远不会返回结果。 |
...
| 刷新受众过期日期 | 13 | 系统强制的护栏 | 自创建之日起，受众将停止刷新的最大月数。 客户能够将这一期限再延长13个月。 |
