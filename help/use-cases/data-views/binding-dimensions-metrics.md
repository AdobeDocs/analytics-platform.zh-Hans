...
进入 **[!UICONTROL 数据视图]** 并绑定 [!DNL Product Color] 维度至 [!DNL Product Name]：
...
如果您使用的分配模型不包含与搜索词的绑定维度，则所有三种产品都将收入归因于单个搜索词。例如，如果您使用 [!UICONTROL 原有] 使用搜索词维度进行分配：

| search_term | 收入 |
| --- | --- |
| 拳击手套 | $204.97 |

如果您使用 [!UICONTROL 最近] 将分配归入搜索词维度，则所有三个产品仍会将收入归因于单个搜索词：
...
您现在可以绑定 [!DNL Search Term] 到 [!DNL Product Name] 只要 [!DNL Searches] 存在量度以正确地将搜索词归因于收入。
