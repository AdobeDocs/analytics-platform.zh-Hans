...
的 [Analytics源连接器](https://experienceleague.adobe.com/docs/experience-platform/sources/ui-tutorials/create/adobe-applications/analytics.html?lang=zh-Hans) 提供了一种将报表包数据从Adobe Analytics导入Adobe Experience Platform以供AEP应用程序（如RTCDP和CJA）使用的方法。 引入AEP的每个报表包都配置为单个源连接数据流，并且每个数据流都作为数据集登陆AEP数据湖中。 （Analytics源连接器将为每个报表包创建一个数据集。）
...
这种情况将导致eVar1和eVar2的无意义报告：

- eVar字段将包含具有不同语义的值的混合。
- 搜索词将在eVar1和eVar2之间分发。
- 将无法对每个搜索词、业务单位和客户类别使用不同的归因模型。

## 使用AEP数据准备解决报表包之间的架构差异，以在CJA中使用

AEP的数据准备功能与Analytics源连接器集成，可用于解决上述情景中所述的架构差异，从而在CJA数据视图中生成具有一致含义的eVar。 下面是如何实现此目标的描述。 您可以根据自己的需要自定义下面使用的命名约定。

在为报表包A和报表包B创建源连接数据流之前，请在AEP中创建自定义字段组(我们将其称为 **统一字段** 在我们的示例中)，其中包含以下字段：
...
在AEP中创建新架构(我们将其称为 **统一模式** )。 将以下字段组添加到架构中：
...
创建的源连接数据流时 **报表包A**，选择 **统一模式** 用于数据流。 按如下方式添加自定义映射：
...
注意：目标字段的XDM路径将取决于您如何设置自定义字段组。

创建的源连接数据流时 **报表包B**，再次选择 **统一模式** 用于数据流。 工作流将显示两个字段存在描述符名称冲突。 这是因为eVar1和eVar2的描述符在报表包B中与在报表包A中不同。但我们已经知道这一点，因此我们可以安全地忽略冲突并使用如下自定义映射：
...
现在创建 **所有报表包** 连接（数据集A和数据集B的组合）。

创建 **全局视图** 数据视图。 忽略原始eVar字段，并仅包含统一字段字段字段组中的字段。
...
实际上，您现在已将eVar1和eVar2从源报表包映射到三个新字段。 请注意，使用数据准备映射的另一个优势是，目标字段现在基于语义上有意义的名称（搜索词、业务部门、客户类别），而不是较不有意义的eVar名称(eVar1、eVar2)。

注意：可以随时将统一字段自定义字段组和关联的字段映射添加到现有的Analytics源连接器数据流和数据集，但这将仅影响将来的数据。
...
使用数据准备，您可以将Analytics数据中eVar1中的客户类别与呼叫中心数据中Some_field中的客户类别组合。 你可以这样做。 再次重申，可以根据您的需要更改命名约定。
...
为创建数据流时 **数据集A**，选择 **扩展架构** 作为您的架构。 按如下方式添加自定义映射：

| 数据集A源字段 | “客户信息”字段组中的目标字段 |
| --- | --- |
| \_experience.analytics.customDimensions.eVars.eVar2 | _\&lt;path>_.Customer_category |

为创建数据流时 **数据集B**，再次选择 **扩展架构** 作为您的架构。 按如下方式添加自定义映射：
...
更改组件ID是仅限CJA的函数，不会影响Analytics源连接器中发送到统一配置文件和RTCDP的数据。
