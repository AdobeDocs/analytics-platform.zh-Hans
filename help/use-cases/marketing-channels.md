...
* 必须已使用 [ Analytics Source Connector ](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/analytics.html) 将报告包数据导入 Adobe Experience Platform。不支持其他数据源，因为“营销渠道”维度依赖于 Analytics 报表包中的处理规则。
...
>[!NOTE]
>
> Analytics源连接器要求 `channel.typeAtSource` （营销渠道）和 `channel._id` （营销渠道详细信息），否则，将不会传递到XDM ExperienceEvent。 如果源报表包中的“营销渠道详细信息”为空，则会导致显示空白 `channel._id` 和Analytics Source Connector将空白 `channel.typeAtSource` 也是。 这可能会导致Adobe Analytics和Customer Journey Analytics之间的报表差异。
