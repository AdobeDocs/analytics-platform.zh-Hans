---
title: 创建或编辑连接
description: 了解如何在Customer Journey Analytics中创建或编辑与Experience Platform数据集的连接。
...
>title="更改身份设置 "
>abstract="一旦在“连接”界面中保存了修改，更改标识设置即会删除此数据集的连接数据。 您将必须根据新设置手动启动数据集的数据回填。<br/><br/>在删除过程完成之前，报告可能会延迟。"
...
>title="使用主要身份标识命名空间"
>abstract="在每个事件级别自动检测并使用定义为&#x200B;**[!UICONTROL primary]**&#x200B;的identityMap命名空间。"
>additional-url="https://experienceleague.adobe.com/zh-hans/docs/experience-platform/identity/features/namespaces" text="身份标识命名空间概述"
...
>title="从 Customer Journey Analytics 中移除此连接"
>abstract="此产品使用情况连接当前用于Customer Journey Analytics。 通过连接，您可以：<ul><li>在 Customer Journey Analytics 中对产品使用情况数据进行深入分析。</li><li>编辑产品使用情况连接和相关数据视图。</li></ul>**如果您移除此连接，就无法在 Customer Journey Analytics 中进行深入分析。此连接和任何相关联的数据视图都被重置为其默认状态，无法再编辑。**<br/><br/>**Customer Journey Analytics 中此连接的计费包括此连接被移除的整月。**<br/><br/>&#x200B;此连接为产品使用情况保持启用状态。[了解详情](https://experienceleague.adobe.com/zh-hans/docs/analytics-platform/using/tools/product-usage/usage-overview){target="_blank"}。"
...
1. 配置连接设置。
   ...
   | **[!UICONTROL 已拼接]** | 如果数据集[在连接 UI 中启用了拼接](/help/stitching/use-stitching-ui.md)，则该值为&#x200B;**[!UICONTROL 真]**。 否则，值为&#x200B;**[!UICONTROL false]**。 通过[请求拼接流程](/help/stitching//use-stitching.md)生成的已拼接数据集不会在此表中标识为已拼接，默认值为&#x200B;**[!UICONTROL 假]**。 |
...
>title="标识拼接"
>abstract="身份拼接从数据集或身份图获取用户身份信息。 此过程可能包括将来自已验证和未验证会话的用户数据合并起来。<br/><br/>您有责任遵守适用的法律和法规。此合规要求包括在对数据集启用拼接之前，必须获得必要的最终用户授权。"
>additional-url="https://experienceleague.adobe.com/zh-hans/docs/analytics-platform/using/stitching/overview" text="拼接概述"
...
>[!AVAILABILITY]
>
>本节中介绍的增强&#x200B;**[!UICONTROL 数据集预览]**&#x200B;界面处于版本的有限测试阶段，可能尚未在环境中可用。 如果不可用，您将看到数据集预览作为&#x200B;**[!UICONTROL 数据集设置]**&#x200B;界面的一部分。 当该功能正式发布时，将删除此说明。有关 Customer Journey Analytics 发布流程的信息，请参阅 [Customer Journey Analytics 功能版本](/help/release-notes/releases.md)。
>
...
1. 在&#x200B;**[!UICONTROL 添加数据集]**&#x200B;对话框的➋ **[!UICONTROL 数据集设置]**&#x200B;步骤中，一个接一个地配置[每一个选定数据集的设置](#dataset-settings)。
   ...
   * 要后退一步，请选择&#x200B;**[!UICONTROL 后退]**。
   * 要取消向连接添加数据集，请选择&#x200B;**[!UICONTROL 取消]**。

1. 您需要为所选数据集指定所有必需的设置，然后才能继续。 如果缺少所需的输入，您将看到一个红色数字，指示有多少特定类型的数据集缺少所需的输入。 ![AlertRed](/help/assets/icons/AlertRed.svg)和红色说明标识没有输入或选定值的必填字段。 配置所有数据集所需的所有设置后，选择&#x200B;**[!UICONTROL 下一步]**。
1. 在❸ **[!UICONTROL 数据集预览]**&#x200B;中，您会看到每个数据集的预览，该预览基于最近摄取数据的简单数据集。

   将数据集添加为连接的一部分时![预览数据集](/help/connections/assets/add-dataset-preview.png)
...
1. 选择&#x200B;**[!UICONTROL 添加数据集]**&#x200B;以将配置的数据集添加到连接。
...
1. 在&#x200B;**[!UICONTROL 添加数据集]**&#x200B;对话框的➋ **[!UICONTROL 数据集设置]**&#x200B;步骤中，一个接一个地配置[每一个选定数据集的设置](#dataset-settings)。
   ...
   ![添加数据集](assets/add-dataset-b2b.png)
...
1. 选择![编辑](/help/assets/icons/Edit.svg) **[!UICONTROL 编辑连接]**。
1. 为要编辑的数据集表中列出的数据集选择![更多](/help/assets/icons/More.svg)。
1. 选择![编辑](/help/assets/icons/Edit.svg) **[!UICONTROL 编辑数据集]**。

1. 在❶ **[!UICONTROL 数据集设置]**&#x200B;中，在[编辑数据集：](#dataset-settings)数据集名称&#x200B;**[!UICONTROL 对话框中配置&#x200B;_数据集设置_]**。

   ![编辑数据集](assets/edit-dataset-b2c.png)

   如果进行了更改，请确保为数据集指定所有必需的设置，然后再继续。 如果缺少所需的输入，则无法继续。 ![AlertRed](/help/assets/icons/AlertRed.svg)和红色说明标识没有输入或选定值的必填字段。
   ...
   配置数据集所需的所有设置后，选择&#x200B;**[!UICONTROL 下一步]**。
