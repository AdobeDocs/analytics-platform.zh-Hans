...
| ➍ | [在连接中添加新数据集](create-connection.md#add-datasets)。 您的连接至少应该有一个事件或摘要数据集，但可以包含各种事件、轮廓、查找和摘要数据集。 |
...
| ➏ | [编辑现有数据集的设置](create-connection.md#edit-a-dataset)。 您始终可以在稍后的阶段重新查看数据集设置。 |



## 访问控制

连接管理的访问权限应仅限于核心管理组。 连接配置会对导入 Customer Journey Analytics 的数据量配额产生合同方面的影响。
