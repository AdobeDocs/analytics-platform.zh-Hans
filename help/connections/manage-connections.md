...
1. 选择蓝色操作栏中的![编辑](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Edit_18_N.svg) **[!UICONTROL 编辑]**。
...
1. 选择蓝色操作栏中的![删除](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Delete_18_N.svg) **[!UICONTROL 删除]**。

删除连接时，**[!UICONTROL 删除连接]**&#x200B;面板会指示删除的是哪些数据视图，以及哪些工作区项目会受到影响。

* 在➊ **[!UICONTROL 信息]**&#x200B;中显示了删除连接的后果。

  ![删除连接](assets/delete-connection.png)

  选择&#x200B;**[!UICONTROL 继续]**，确认删除。

* 在➋ **[!UICONTROL 确认]**&#x200B;中，在&#x200B;**[!UICONTROL 输入连接名称]**&#x200B;中输入连接的名称，然后选择&#x200B;**[!UICONTROL 删除]**&#x200B;将该连接删除。选择&#x200B;**[!UICONTROL 取消]**&#x200B;即可取消。
...
1. 选择蓝色操作栏中的![添加数据视图](https://spectrum.adobe.com/static/icons/workflow_18/Smock_DataAdd_18_N.svg) **[!UICONTROL 创建数据视图]**。
...
* 编辑 Journey Optimizer 连接和相关联的数据视图。
  ...
  关于编辑选项的详细信息请参阅[编辑连接](#edit-a-connection)。
...
要启用此功能，您的组织需要 Customer Journey Analytics 的访问权限。如果您没有访问权限，请联系您的 Adobe 销售代表。

#### 使用 Journey Optimizer 连接 {#use-connection-in-cja}

要在 Customer Journey Analytics 中使用 Journey Optimizer 连接：

1. 找到要用于 Customer Journey Analytics 的 Journey Optimizer 连接。

   1. 在&#x200B;**[!UICONTROL 连接]**&#x200B;选项卡中选择![过滤器](/help/assets/icons/Filter.svg) **[!UICONTROL 过滤器]**。

   1. 在&#x200B;**[!UICONTROL 在 CJA 中使用]**&#x200B;部分，选择 **[!UICONTROL Off]**。

      这会显示当前所有未配置为可在 Customer Journey Analytics 中使用的 Journey Optimizer 连接。

      ![筛选显示没有为 AJO 启用的连接](assets/remove-ajo-connection.png)

1. 选择 Journey Optimizer 连接的名称。

1. 选择 ![UsersShare](/help/assets/icons/UseInCJA.svg) **[!UICONTROL 在 CJA 中使用]**。

   ![在 CJA 中使用按钮](assets/connection-use-in-cja.png)

   现在会显示&#x200B;**[!UICONTROL 在 Customer Journey Analytics 中使用这个连接]**&#x200B;对话框。

1. 启用切换功能，**[!UICONTROL 在 CJA 中使用连接]**。

1. 选择&#x200B;**[!UICONTROL 使用连接]**。<!-- double-check these dialog button names -->

#### 移除 Journey Optimizer 连接 {#remove-connection-in-cja}

您可以随时从 Customer Journey Analytics 中移除 Journey Optimizer 连接。但是，在使用连接后从 Customer Journey Analytics 中移除连接会导致以下情况：

* Journey Optimizer 连接和任何相关联的数据视图都被重置为其默认状态，无法再编辑。

* 与此连接相关联的任何自定义派生字段都被删除。

* 您无法在 Customer Journey Analytics 中对 Journey Optimizer 数据进行深入分析。

  这意味着 Journey Optimizer 中的&#x200B;**[!UICONTROL 在 CJA 中分析]**&#x200B;按钮被禁用。

>[!IMPORTANT]
>
>Customer Journey Analytics 中此连接的计费包括此连接被移除的整月。


要从 Customer Journey Analytics 中移除此连接：

1. 找到要从 Customer Journey Analytics 中移除的 Journey Optimizer 连接。

   1. 在&#x200B;**[!UICONTROL 连接]**&#x200B;选项卡中选择![过滤器](/help/assets/icons/Filter.svg) **[!UICONTROL 过滤器]**。

   1. 在&#x200B;**[!UICONTROL 在 CJA 中使用]**&#x200B;部分，选择 **[!UICONTROL On]**。

      这会显示当前所有配置为可在 Customer Journey Analytics 中使用的 Journey Optimizer 连接。

      ![筛选显示为 AJO 启用的连接](assets/enabled-ajo-connection.png)

1. 要查看连接，请选择要从 Customer Journey Analytics 中移除的 Journey Optimizer 连接的名称。

1. 查看 Journey Optimizer 连接时，选择&#x200B;**[!UICONTROL 从 CJA 中移除]**。

   现在会显示&#x200B;**[!UICONTROL 从 Customer Journey Analytics 中移除此连接]**&#x200B;对话框：

   ![从 CJA 中移除按钮](assets/connection-remove-from-cja.png)

1. 禁用选项，**[!UICONTROL 从 CJA 中移除连接]**。
...
要查看有关连接的详细信息，请在连接表中选择一个有超链接的连接名称。
...
| **[!UICONTROL *数据集选择器&#x200B;*]** | 选择一个或多个数据集，显示连接中的详细信息。不能选择其他数量的数据集。默认为&#x200B;**[!UICONTROL 全部数据集]**。 |
...
| **[!UICONTROL 数据集 ID]** | Experience Platform 生成的数据集 ID。 |
| **[!UICONTROL 添加的记录数]** | 在所选日期范围内添加到某个连接的数据集记录数（行数）。 |
| **[!UICONTROL 跳过的记录数]** | 在所选日期范围内为某个连接传输数据期间跳过的数据集记录数（行数）。 |
| **[!UICONTROL 删除的记录数]** | 在所选日期范围内从某个连接移除的数据集记录数（行数）。 |
| **[!UICONTROL 添加的批次数]** | 在选定日期范围内添加到某个连接的批次数。 |
| **[!UICONTROL 上次添加时间]** | 已添加到连接的最新批次的时间戳。 |
| **[!UICONTROL 数据源类型]** | 来源类型。您在向连接添加数据集时定义来源类型。 |
| **[!UICONTROL 数据集类型]** | [数据集类型](create-connection.md#dataset-types)。类型可以是&#x200B;**[!UICONTROL 事件]**、**[!UICONTROL 轮廓]**、**[!UICONTROL 查找]**&#x200B;或&#x200B;**[!UICONTROL 摘要]**。临时或关系数据集通过&#x200B;**[!UICONTROL （临时）]**&#x200B;或&#x200B;**[!UICONTROL （关系）]**&#x200B;标识。例如，**[!UICONTROL 事件（临时）]**&#x200B;或&#x200B;**[!UICONTROL 查找（关系）]**。 |
| **[!UICONTROL 已拼合]** | 如果在连接UI[中启用了数据集](/help/stitching/use-stitching-ui.md)的拼接功能，则值为&#x200B;**[!UICONTROL true]**。 否则，值为&#x200B;**[!UICONTROL false]**。 此表中未将拼接过程[的](/help/stitching//use-stitching.md)请求所生成的拼接数据集标识为已拼接，默认值为&#x200B;**[!UICONTROL false]**。 |
...
如果在数据集表中未选择任何数据集，右侧面板就会显示连接选项和详细信息。
...
| **[!UICONTROL 上次修改人]** | 最后一次更改连接的人员的姓名。 |
...
| **[!UICONTROL 数据源类型]** | 数据集添加到连接时定义的数据源类型。 |
| **[!UICONTROL 数据集类型]** | [数据集类型](create-connection.md#dataset-types)。类型可以是&#x200B;**[!UICONTROL 事件]**、**[!UICONTROL 轮廓]**、**[!UICONTROL 查找]**&#x200B;或&#x200B;**[!UICONTROL 摘要]**。临时或关系数据集通过&#x200B;**[!UICONTROL （临时）]**&#x200B;或&#x200B;**[!UICONTROL （关系）]**&#x200B;标识。例如，**[!UICONTROL 事件（临时）]**&#x200B;或&#x200B;**[!UICONTROL 查找（关系）]**。 |
| **[!UICONTROL 架构]** | 此数据集所基于的 Experience Platform 架构。 |
| **[!UICONTROL 数据集 ID]** | 在 Experience Platform 中生成的数据集 ID。 |
