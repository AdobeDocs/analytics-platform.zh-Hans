...
* 不支持在组织中使用的自定义 ID 映射。
* 不支持 Adobe 协作图和专用图。
* 跨渠道 Analytics 不以任何方式转换用于拼接的字段。基于字段的拼接使用存在于数据湖内非拼接数据集中的指定字段中的值。触摸过程区分大小写。 例如，如果字段中有时出现“Bob”一词，有时出现“BOB”一词，则将这两个词视为单独的两人。
* 给定的基于字段的拼接区分大小写，对于通过Analytics数据连接器生成的Analytics数据集，Adobe建议查看适用于临时ID字段的任何VISTA规则或处理规则，以确保这些规则中没有一种引入新形式的相同ID。 例如，您应确保没有VISTA或处理规则将小写引入仅部分事件上的临时ID字段。
* 基于字段的拼接不会合并或连接字段。
* 临时ID字段应包含单一类型的ID(即来自单个命名空间的ID)。 例如，临时ID字段不应包含登录ID和电子邮件ID的组合。
* 如果同一永久ID的时间戳相同，但瞬时ID字段中的值不同，则基于字段的拼接将根据字母顺序进行选择。 因此，如果永久ID A有两个具有相同时间戳的事件，其中一个事件指定Bob，另一个指定Ann，则基于字段的拼接将选择Ann。
...
1. Adobe 客户经理将与 Adobe 工程部门合作，以便在您要求时启用跨渠道 Analytics。一旦启用，Adobe Experience Platform 中就会出现一个更新了密钥的新数据集，其中包含新的人员 ID 列。您的 Adobe 客户经理可提供新的数据集 ID 和人员 ID 列名称。
