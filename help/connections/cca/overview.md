...
**历程IQ:跨渠道分** 析是一项功能，它允许您重新键入数据集的个人ID，从而实现多个数据集的无缝组合。CCA 将同时考虑来自经过身份验证和未经身份验证的会话的用户数据，以生成拼合 ID。使用跨渠道分析，您可以回答以下问题：
...
1. Adobe客户经理在收到您的请求后启用跨渠道分析。 启用后，Adobe Experience Platform 中会显示一个新的已重新生成键值的数据集，其中包含一个新的“人员 ID”列。Adobe 客户经理可以提供新的数据集 ID 和“人员 ID”列名称。
