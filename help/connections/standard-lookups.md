...
>[!IMPORTANT]
>标准查找仅适用于CJA中的Analytics源连接器数据源。 您可以将其用于标准 Adobe Analytics 实施、[Adobe Experience Platform Web SDK](https://experienceleague.adobe.com/docs/experience-platform/edge/home.html) 或 Experience Platform 数据收集 API。
...
## 将标准查找与Adobe源连接器数据集结合使用

标准查找数据集在报告时自动应用。如果您使用Analytics源连接器并引入了Adobe为其提供标准查找的维度，则我们会自动应用此标准查找。 如果某个事件数据集包含 XDM 字段，我们可以对其应用标准查找。
