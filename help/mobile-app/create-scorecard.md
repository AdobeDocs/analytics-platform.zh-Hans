...
1. 在&#x200B;**[!UICONTROL 属性]**（位于右侧边栏中）下，指定要从中使用数据的&#x200B;**[!UICONTROL 项目报表包]**。报表包是CJA中的数据视图。
...
要将过滤器应用于单个图块，请从左侧面板中拖动过滤器（区段是CJA中的过滤器），并将其直接拖放到图块的顶部。
...
与饼图类似，此可视化图表将数据显示为整体的一部分。 在比较总数的百分比时使用圆环图。例如，假设您想查看哪个广告平台对独特访客总数做出了贡献：
...
在记分卡生成器中单击图块时，右侧边栏会显示与该图块及其详细信息幻灯片关联的属性和特征。 在此边栏中，您可以提供 **标题** ，或者通过应用过滤器来配置图块。 区段是CJA中的过滤器。

![“属性”图块](assets/properties-tile-new.png)

## 查看详细幻灯片 {#view-detail-slides}

单击图块时，动态弹出窗口会显示详细幻灯片在应用程序中向执行用户显示的方式。 您可以添加维度来根据您的特定需求划分数据。 如果未应用维度，则划分维度将 **小时** 或 **天数**，具体取决于默认日期范围。

划分可通过按维度项目划分量度来优化分析，例如：
...
通过自定义详细信息幻灯片，您可以更加有针对性地了解与受众共享的哪些信息。 您可以修改每个详细幻灯片的布局并添加文本，以便更好地说明最终用户在数据中可能看到的内容。 您还可以使用下拉菜单更改图表类型。

![自定义详细信息幻灯片](assets/custom-detail-slide.png)

### 更改幻灯片布局

更改幻灯片布局以重点关注最重要的信息。 例如，您可以更改布局以仅显示一个图表或一个表格。 要更改幻灯片布局，请选择预先设计的格式之一。

![幻灯片布局](assets/layout.png)

您还可以通过将可视化组件从左边栏拖放到画布上来更改幻灯片布局。 每张详细信息幻灯片一次只能显示两个可视化图表。

![幻灯片布局更改](assets/slide-layout-change.png)

### 在幻灯片中添加描述性文本

您可以添加文本，以提供有关图表中包含的内容或数据细微差别的有意义信息。

要向详细信息幻灯片添加文本，请选择一个布局，该布局显示 `T` 符号，或从左边栏将文本可视化组件拖放到上方。 添加新的文本可视化图表或选择带有文本的幻灯片布局时，将自动打开文本编辑器。 文本编辑器提供了用于设置文本格式的所有标准选项。 您可以应用文本样式（如段落、标题和子标题），并应用粗体和斜体字体。 您可以对齐文本、添加项目符号和编号列表以及添加链接。 编辑完成后，选择文本编辑器右上角的最小化按钮以将其关闭。 要编辑已添加的文本，请选择铅笔图标以再次打开文本编辑器。

![幻灯片布局更改](assets/add-descriptive-text.png)

## 删除组件 {#remove}

同样，要删除应用于整个记分卡的组件，请单击记分卡上图块外的任意位置，然后单击 **x** 当您将鼠标悬停在组件上时显示，如下 **首次访问**:
