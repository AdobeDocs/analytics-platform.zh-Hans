...
* 在数据视图用户界面中，指定默认属性。 **注意**:在以后的日期，您将能够覆盖Workspace项目中的这些设置。 但是，此功能当前不可用。

* 客户旅程分析中的归因设置是无损的，并具有追溯性。 换句话说，您不能对客户旅程分析中的数据集造成不可挽回的伤害。 即使您意外删除了某些内容，您也始终可以返回 [!UICONTROL Experience Platform] 并将数据集重新导入。 （但是，请记住，重新引入数据集将会产生额外成本。）
...
设置并保 [存数据视图设置和添加组件后](/help/data-views/create-dataview.md) ，如果选择指定属性设置，您可以随时指定。 您可以为维度和指标指定归因／到期／回顾设置。 例如，如果您希望维的属性持续存在，您可能需要设置自定义的到期时间。 例如，如果您希望将“跟踪代码”维度(活动变量)设置为“最近联系”属性的时间持续一周，请添加1周的自定义过期时间。
...
该模型描述了转化到组中事件的分布情况。 例如，首次接触或最后接触。确定当变量在事件之前收到多个值时，客户旅程分析如何分配成功事件的信用。
...
| 设置 | 定义 |
|---|---|
| Session | 以前称为“访问”级别。 超出页面视图或会话范围的转换事件不与维度或度量相关联。 |
| 人员(报告窗口) | 以前称为“访客”级。 未绑定到此人的转化事件不与维度或量度关联。 |
| 自定义时间 | 指定自定义分钟数、小时数、天数、月数或季度数。 超出指定时间段的转换事件不与维度或度量相关联。 |
