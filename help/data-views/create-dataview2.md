...
创建视图涉及从模式元素创建量度和维度，或使用标准组件。 创建量度或维度为您提供了极大的灵活性。 以前，假设您在Adobe Experience Platform中有数据集，字符串字段是维度，数字字段是量度。 要更改其中的任何字段，您必须在平台中编辑模式。 数据视图UI现在允许对量度和维度进行更自由的定义。
...
| 设置 | 描述/用例 |
| --- | --- |
| 连接 | 此字段将数据视图链接到您之前建立的连接，该连接包含一个或多个Adobe Experience Platform数据集。 |
| 名称 | 必须为数据视图命名。 |
| 描述 | 详细说明不是强制性的，但建议提供。 |
| 时区 | 选择要显示数据的时区。 |
| 标记 | 标记让您可以将数据视图组织成不同的类别。 |
| 容器 | 您可以在此处重命名容器，这就是它们在基于此数据视图的任何Workspace项目中的显示方式。 容器用于过滤器和流失/流，以定义范围或上下文的范围或范围。 [了解详情](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-components/cja-filters/filters-overview.html?lang=en#filter-containers) |
| 人员容器名称是…… | 人员（默认）。 人员容器包含指定时间段内访客的每次访问和页面查看。您可以将其重命名为“用户”或您喜欢的任何其他术语。 |
| 会话容器名称为…… | 会话（默认）。 通过会话容器可以识别页面交互、促销活动或特定会话的转化。您可以将其重命名为“访问”或您喜欢的任何其他术语。 |
| 事件容器名称为…… | 事件（默认）。 事件容器定义要在过滤器中包含或排除的页面事件。 |

接下来，您将从模式元素创建量度和维度。

## 从模式元素创建量度和维度

1. 在[!UICONTROL 客户历程分析] > [!UICONTROL 视图]中，单击[!UICONTROL 组件]选项卡。
...
1. 现在，将模式字段（如[!UICONTROL pageTitle]）从左边栏拖动到“量度”或“Dimension”部分。
   ...
   您可以多次将同一模式字段拖动到维度或量度中，并以不同方式配置相同的维度或量度。 例如，从&#x200B;**[!UICONTROL pageTitle]**&#x200B;字段中，可以创建名为“Product Pages”的维度，以及另一个“Error pages”，等等。 从&#x200B;**[!UICONTROL pageTitle]**;字段中，您还可以根据字符串值创建量度。 例如，您可以创建一个或多个具有不同属性设置和不同包含/排除值的&#x200B;**[!UICONTROL Orders]**&#x200B;量度。
...
| 路径 | 必需. 一个不可编辑的字段，显示组件来自的模式路径。 |
| 在报表中隐藏组件 | 默认=关闭。 用于在报告中使用时，可将组件从“视图”中管理出来。 这不会影响权限，只影响组件特选。 换言之，您可以在报告中对非管理员隐藏组件。 管理员仍可通过单击Analysis Workspace项目中的[!UICONTROL 显示所有组件]来访问它。 |
...
重要说明：将此字段更改为自定义值时，自定义值将被视为合法字符串值。 因此，如果在此字段中输入值“Red”，则数据本身中出现的字符串“Red”的任何实例也将滚动到您指定的同一行项目下。

另请注意，无论您在此字段中指定什么，都可以用于“无值选项”设置中所述的报告中“无值”行项目的特殊UI处理。 （不确定这意味着什么。）

![](assets/no-value-options.png)

| 设置 | 描述/用例 |
| --- | --- |
| 如果显示，调用[!UICONTROL 无值]... | 您可以在此处将&#x200B;**[!UICONTROL 无值]**&#x200B;重命名为其他值。 |
| 默认情况下不显示&#x200B;**[!UICONTROL 没有值]** | 不在报告中显示此值。 |
| 默认情况下显示&#x200B;**[!UICONTROL 无值]** | 在报告中显示此值。 |
| 将&#x200B;**[!UICONTROL 没有值]**&#x200B;视为值 | 例如，如果您将移动设备类型作为维度，则可以将&#x200B;**[!UICONTROL No value]**&#x200B;项重命名为“Desktop”。 |
...
| --- | --- |
| 设置持久性 | 切换键 |
| 分配 | 允许您指定用于持久性的维的分配模型。 选项有：最近，原始，实例，全部。 如果您希望某个值持续存在（与传统Analytics中的eVar类似），您将在此处设置它。 唯一的关键区别是您可以设置的最大持久性为90天。 此外，[!UICONTROL 不过期]不是选项。 |
| 过期 | 用于指定维的持久性窗口。 选项有：会话（默认）、人员、时间、量度。您可能需要能够使购买的维度过期（例如内部搜索词或其他促销用例）。 “量度”允许您指定任何定义的量度作为此维度的过期时间（例如“购买”量度）。 |
...
| 设置 | 描述/用例 |
| --- | --- |
| 分段值 | 允许您创建数字维度的分时段版本。 这允许您将收入时段或其他数值作为报告中的维度进行报告。 最多可创建5个桶。 |
| 最高 | 允许您指定第一个数字维度时段的边界。 这仅适用于数字维。 |
| 介于 | 允许您指定后续数字维度时段的边界。 |
| 添加分段 | 允许您向数字维度分段添加另一个时段。 |

## 使用标准组件

除了从模式元素创建量度和维度之外，您还可以在数据视图中使用标准组件。

标准组件是不是从数据集模式字段生成，而是系统生成的组件。 任何数据视图都需要某些系统组件，以便在Analysis Workspace中实现报告功能，而其他系统组件是可选的。
...
| --- | --- | --- |
| 人员 | 指标 | 传统Analytics中以前称为[!UICONTROL 唯一访客]。 此量度基于在连接中指定的人员ID。 |
| Sessions | 指标 | 传统Analytics中以前称为[!UICONTROL 访问]。 此量度基于以下指定的会话化设置。 |
| 事件 | 指标 | 在传统Analytics中，以前称为[!UICONTROL Occurrences]。 此量度表示连接中所有事件数据集的行数。 |
| 日 | 维度 |  |
| 周 | 维度 |  |
| 月 | 维度 |  |
| 季度 | 维度 |  |
