---
title: BI扩展Customer Journey Analytics
description: 了解如何使用Query Service、Power BI、Tableau或其他BI和SQL工具通过Customer Journey AnalyticsBI扩展访问数据视图。
...
# BI扩展Customer Journey Analytics
...
* 无需在 BI 工具中重新创建 Customer Journey Analytics 数据视图的等效表示形式。<br/>请参阅 [数据视图](data-views.md) 有关数据视图功能的更多信息，以了解必须重新创建的内容。
...
* 为相关的产品配置文件、用户组和/或个人用户配置功能。 访问要求包括：
   * Adobe Experience Platform查询服务
   * Customer Journey Analytics中的工作区项目
   * 要使用的所需CJA数据视图
   * 在数据视图工具中访问BI扩展

* 使用过期凭据将BI工具连接到 [!DNL Customer Journey Analytics BI extension]. 此 [凭据指南](https://experienceleague.adobe.com/en/docs/experience-platform/query/ui/credentials) 提供了有关设置过期凭据或不过期凭据的详细信息。
...
1. 要执行查询，请键入您的SQL语句并选择 ![播放](assets/Smock_Play_18_N.svg) 按钮(或按 `[SHIFT]` + `[ENTER]`)。
...
1. 在Adobe Experience Platform中查找并复制您的PostgresSQL凭据：
   ...
   1. 要复制命令字符串，请使用 ![复制](assets/Smock_Copy_18_N.svg) 在 **[!UICONTROL ** PSQL命令&#x200B;**]** 部分。

1. 打开命令或终端窗口。

1. 要登录并开始执行查询，请在终端中粘贴命令字符串。
...
目前， [!DNL Customer Journey Analytics BI extension] 仅支持Power BI和Tableau并进行测试。 其他使用PSQL接口的BI工具也可能正常工作，但尚未得到正式支持。

+++ Power BI

1. 在Adobe Experience Platform中查找PostgresSQL凭据的详细信息：
...
1. 在 Power BI 中：
   ...
   1. 在 **[!UICONTROL ** PostgressSQL 数据库&#x200B;**]**&#x200B;对话框中：
      ...
      1. 提示输入时 **[!UICONTROL 数据连接]** 模式，选择 **[!UICONTROL Directquery]**.

      1. 系统会提示您输入&#x200B;**[!UICONTROL 用户名]**&#x200B;和&#x200B;**[!UICONTROL 密码]**。使用 Experience Platform 查询[!UICONTROL 凭据]中的等效参数。


   1. 成功登录后，Customer Journey Analytics数据视图表将出现在Power BI的 **[!UICONTROL **&#x200B;导航器&#x200B;**]**.
...
1. 在 Tableau 中：
   ...
   1. Customer Journey Analytics数据视图在中显示为表 **[!UICONTROL **&#x200B;表&#x200B;**]** 列表。
...
默认情况下，您的数据视图具有从其友好名称生成的表安全名称。例如，名为的数据视图 [!UICONTROL 我的Web数据视图] 具有视图名称 `my_web_data_view`.
...
在 Experience Platform 使用的数据集上创建的隐私标签和政策可以在 Customer Journey Analytics 数据视图工作流中显示。因此，使用 [!DNL Customer Journey Analytics BI extension] 在不遵循所定义的隐私标签和策略时，显示相应的警告或错误。
...
| 子选择：<br/>筛选其他<br/>个结果 | <pre>SELECT dim1, m1<br/>FROM (<br/>  SELECT dim1, SUM(metric1) AS m1<br/>  FROM dv1<br/>  WHERE \`timestamp\` BETWEEN &#39;2022-01-01&#39; AND &#39;2022-01-02&#39;</br>  GROUP BY dim1<br/>)<br/>WHERE dim1 in (&#39;A&#39;, &#39;B&#39;)</pre> |
| 子选择：<br/>正在跨以下位置查询<br/>数据视图 | <pre>SELECT key, SUM(m1) AS total<br/>FROM (<br/>  SELECT dim1 AS key, SUM(metric1) AS m1<br/>  FROM dv1<br/>  WHERE \`timestamp\` BETWEEN &#39;2022-01-01&#39; AND &#39;2022-01-02&#39;<br/>  GROUP BY dim1<br/><br/>  UNION<br/><br/>  SELECT dim2 AS key, SUM(m1) AS m1<br/>  FROM dv2<br/>  WHERE \`timestamp\` BETWEEN &#39;2022-01-01&#39; AND &#39;2022-01-02&#39;<br/>  GROUP BY dim2<br/>GROUP BY key<br/>ORDER BY total</pre> |
| 子选择： <br/>分层源， <br/>正在筛选， <br/>和聚合 | 使用子选择进行分层：<br><pre>SELECT rows.dim1, SUM(rows.m1) AS total<br/>FROM (<br/>  SELECT \_.dim1,\_.m1<br/>  FROM (<br/>    SELECT \* FROM dv1<br/>    WHERE \`timestamp\` BETWEEN &#39;2022-01-01&#39; AND &#39;2022-01-02&#39;<br/>  ) \_<br/>  WHERE \_.dim1 in (&#39;A&#39;, &#39;B&#39;, &#39;C&#39;)<br/>) rows<br/>GROUP BY 1<br/>ORDER BY total</pre><br/>使用 CTE WITH 的层：<br/><pre>行为(<br/>  带有\_ AS (<br/>    选择*从数据区(_A)<br/>    其中\&#39;timestamp\&#39;介于&#39;2021-01-01&#39;和&#39;2021-02-01&#39;之间<br/>  )<br/>  选择\_.item， \_.units FROM \_<br/>  其中\_.item不为NULL<br/>)<br/>SELECT rows.item， SUM(rows.units) AS units<br/>FROM行，其中rows.item位于(&#39;A&#39;， &#39;B&#39;， &#39;C&#39;)<br/>GROUP BY rows.item</pre> |
...
* `SELECT COUNT(DISTINCT dimension)` 或 `SELECT APPROX_COUNT_DISTINCT(dimension)` 对某个维度的近似不同值计数。有关详细信息，请参阅 [对非重复值计数](#counting-distinct-values).
...
您可以将其他数学应用于中的量度表达式 `SELECT` 而不是在计算量度中定义数学。 下表列出了支持的表达式类型。
...
* 如果只提供最大值(`timestamp < X` 或 `timestamp <= X`)，其范围是从X减去30天到X。
* 如果未提供任何内容，则范围从现在减去30天到现在。

时间戳范围会在RankedRequest中转换为日期范围全局过滤器。
时间戳字段还可用于日期/时间函数，以解析或截断事件时间戳。

#### 日期范围

`daterange` 特殊列的工作方式与 `timestamp` 的类似，但是过滤仅限于完整天。`daterange` 也是可选的，并具有与 `timestamp` 相同的范围默认值。
此 `daterange` 字段还可用于日期/时间函数，以解析或截断事件日期。

此 `daterangeName` 特殊列可用于使用命名日期范围筛选查询，例如 `Last Quarter`.

#### 筛选器ID

`filterId` 特殊列是可选的，用于将外部定义的过滤器应用于查询。将外部定义的过滤器应用于查询类似于将过滤器拖动到 Workspace 中的面板上。可使用 `AND` 将多个过滤器 ID 联接来提供这些 ID。

以及 `filterId`，您可以使用 `filterName` 以使用过滤器的名称而不是ID。

### Where子句

此 `WHERE` 子句分三步处理：

1. 从以下位置查找日期范围： `timestamp`， `daterange`，或 `daterangeName` 特殊字段。

1. 查找任何外部定义的 `filterId`s或 `filterName`s以包含在筛选中。

1. 将剩余的表达式转变为临时过滤器。

通过解析 `WHERE` 子句中的 `AND` 的第一层来进行处理。每个顶层 `AND`-ed表达式必须匹配以上任意一个。 比 `AND` 的第一层更深入的任何项目（或在 `WHERE` 子句在顶层使用 `OR` 时）将处理为临时过滤器。
...
| 函数 | 示例 | 详细信息 |
|---|---|---|
| [演员阵容](https://spark.apache.org/docs/latest/api/sql/index.html#cast) | ``CAST(`timestamp` AS STRING)`` 或 <br/> `` `timestamp`::string `` | 虽然当前不支持类型转换，但不会引发错误。`CAST` 函数将被忽略。 |
...
| [结束时间戳](https://spark.apache.org/docs/latest/api/sql/index.html#to_timestamp) | `` WHERE `timestamp` >= TO_TIMESTAMP('01/01/2022', 'MM/dd/yyyy') AND `timestamp` < TO_TIMESTAMP('01/02/2022', 'MM/dd/yyyy') `` | 将时间字符串解析为时间戳以在 `WHERE` 子句中使用，（可选）并提供该时间字符串的格式。 |
...
| [结束日期](https://spark.apache.org/docs/latest/api/sql/index.html#to_date) | `` WHERE `timestamp` >= TO_DATE('01/01/2022', 'MM/dd/yyyy') AND `timestamp` < TO_DATE('01/02/2022', 'MM/dd/yyyy') `` | 将日期字符串解析为时间戳以在 `WHERE` 子句中使用，（可选）并提供该日期字符串的格式。 |
...
| [日期（已截断）](https://spark.apache.org/docs/latest/api/sql/index.html#date_trunc) | ``SELECT DATE_TRUNC('quarter', `timestamp`)`` | 在传入字段上生成动态维度标识。<br/>支持的字符串粒度为：`'YEAR'`、`'Y'`、`'MONTH'`、`'M'`、`'DAYOFMONTH'`、`'DAY'`、`'D'`、`'DAYOFWEEK'`、`'DOW'`、`'DAYOFYEAR'`、`'DOY'`、`'WEEK'`、`'WOY`、`'W'`、`'QUARTER'`、`'QOY'`、`'Q'`、`'HOUR'` 或 `'MINUTE'`。 |
