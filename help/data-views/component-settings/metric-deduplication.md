...
# 量度重复数据删除组件设置 {#metric-deduplication-component-settings}
...
通过量度重复数据删除，可将量度配置为仅对不重复出现的值进行计数。

![重复量度删除](../assets/metric-deduplication.png)

| 设置 | 描述 |
| --- | --- |
| [!UICONTROL 重复量度删除] | 一个复选框，通过它，可启用量度重复数据删除。 默认禁用。 |
...
| [!UICONTROL 重复数据删除 ID] | 并非对量度本身应用重复数据删除，而是使您可根据维度应用量度重复数据删除。 对于“购买 ID”等维度应用重复数据删除很有用。 |
| [!UICONTROL 要保留的值] | <ul><li>**保留第一个实例**：如果量度的初始实例有效，则使用此项。 最常见的情况可能是购买确认。 即使有人无意中重新加载了页面，使我们得到的是购买确认的另一个实例，但初始事件仍有效。</li><li>**保留最后一个实例**：如果收集最后一个实例更有意义，则使用此项。 示例：某人更新自己的在线轮廓。 我们只需在每个会话中计算这些更新的其中之一。 但是，他们可能会在会话期间多次更新自己的轮廓。 如果我们保留第一个实例，则可能会有无法与该事件关联的活动。 在这种情况下，保留最后一个实例更有意义。</li></ul> |
