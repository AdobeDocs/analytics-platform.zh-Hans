...
| 任务 | 详细信息 |
| --- | --- |
| **步骤1：如果您要从Adobe Analytics升级到Customer Journey Analytics：选择升级路径并将数据发送到Adobe Experience Platform** | 从Adobe Analytics升级到Customer Journey Analytics时，可以使用多种路径。 每种可能的升级路径都有其各自的优缺点，而适合一个组织的路径可能不适合另一个组织。 <p>要开始从Adobe Analytics升级到Customer Journey Analytics，请参阅 [Customer Journey Analytics升级入门](/help/getting-started/cja-upgrade/cja-upgrade-getstarted.md). <!-- [Utilizing Adobe Analytics report suite data in Customer Journey Analytics](/help/getting-started/aa-vs-cja/aa-data-in-cja.md) --> </p> |
