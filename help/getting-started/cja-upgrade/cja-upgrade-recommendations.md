...
1. **实施 Experience Platform Web SDK（用于持续数据收集）**
   ...
   对 Experience Platform Web SDK 进行新的实施是收集 Customer Journey Analytics 数据的最佳方式。 这为充分利用 Customer Journey Analytics 提供了最佳基础，因为这是实施 Customer Journey Analytics 的最高效、最直接、最具前瞻性的方法。

   * Adobe Experience Platform 专为支持实时个性化用例而构建，因此具有高性能报告和数据可用性

   * 整合其他CX Enterprise产品（AJO、RTCDP等）之间的Adobe CX Enterprise数据收集实施

   * 不依赖于 Adobe Analytics 命名法（prop、eVar、事件等）

1. **设置 Adobe Analytics 源连接器（用于传输历史数据）**

   为了帮助顺利过渡到将 Experience Platform Web SDK 与 Customer Journey Analytics 配合使用，Adobe 还建议使用 Adobe Analytics 源连接器。 这样，您就可以在 Customer Journey Analytics 中保留历史数据并查看现有 Adobe Analytics 实施中的数据，同时还能查看新 Experience Platform Web SDK 实施中的数据。
...
每一步都对更详细的流程提供了概括性说明。 按照每个步骤的链接进行操作并完成相关任务，然后返回此页面，并继续执行该流程的下一步。

1. [准备您的组织升级到Customer Journey Analytics](/help/getting-started/cja-upgrade/cja-upgrade-org-readiness.md)。
...
1. 在您的 Adobe Analytics 环境中，[使用 Analytics 库存](https://experienceleague.adobe.com/zh-hans/docs/analytics/admin/admin-tools/analytics-inventory){target="_blank"}了解 Adobe Analytics 环境的全面概述，包括项目和组件的数量、报告包、用户等。
...
1. （可选）如果您使用 Adobe Analytics 中的营销渠道，则可以[在 Customer Journey Analytics 中创建营销渠道派生字段](/help/data-views/derived-fields/derived-fields.md#marketing-channels){target="_blank"}。
   ...
   派生字段是 Customer Journey Analytics 中实时报告功能的一个重要方面。 派生字段允许您通过可自定义的规则生成器动态定义（通常较为复杂的）数据操作。
...
1. [将Adobe Analytics中旧实施的数据与Customer Journey Analytics中新实施的数据进行比较](https://experienceleague.adobe.com/en/docs/cx-enterprise-ai/experience-cloud-ai/coworker/chat/use-cases/data-insights/data-validation-aa-cja)，确保您了解任何差异及其存在原因。
...
1. 完成 Customer Journey Analytics 升级指南。
   ...
   完成此升级指南后，会为您提供包含适合您组织要求的最佳升级步骤的分步说明。 这些升级步骤最符合您现有的 Adobe Analytics 环境以及您在 Customer Journey Analytics 方面的目标。 升级步骤可以作为可共享链接或可下载的 .csv 文件提供。

1. 按照生成的分步说明升级到 Customer Journey Analytics。

### 对于尚无法访问 Customer Journey Analytics 的客户

要为您组织的独特情况动态生成升级步骤：

1. 请联系您的 Adobe 帐户团队以完成 Customer Journey Analytics 升级指南。

   您的 Adobe 帐户团队可以指导您完成升级指南，并为您提供一个 .csv 文件，其中包含问题、您的回答以及为您的组织动态生成的特定升级步骤。
