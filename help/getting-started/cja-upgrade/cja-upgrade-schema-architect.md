...
| 图层 | 主要目的 | 属于什么 | 不属于 |
|---|---|---|---|
| **XDM架构** | 定义所收集数据的持久结构和含义 | 事件和实体形状、字段含义、关系、允许的值、跨渠道重用 | 编号的“插槽”(eVar1/prop1)、归因/持久性逻辑、特定于报表的解决方法 |
| **数据视图** | 定义收集的数据在分析中的行为方式 | 组件设置、归因和持久性行为、派生字段、过滤量度、计算量度 | 字段的基本含义；该含义在架构中应是稳定的 |
...
| Adobe Analytics的常见习惯 | XDM + CJA中更好的方法 |
|---|---|
...
| 设计变量以“预解决”报表 | 设计架构以可靠地捕获事实并使用数据视图解决报告语义 |
...
在Adobe Analytics中，许多团队将`events`变量视为量度的去向。 在Customer Journey Analytics中，您可以通过多种方式为指标建模，具体取决于您需要计数的内容以及您希望如何解释这些内容。
...
>[!TIP]
>
>在决定某个事物是否应是专用字段还是以后的派生字段时，如果持久性事实广泛有用且稳定，则偏好在模式中捕获持久性事实。 您可以使用派生字段在收集后更正数据或调整数据形状。
