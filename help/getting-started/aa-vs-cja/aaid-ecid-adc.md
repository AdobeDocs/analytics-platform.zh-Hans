---
title: AAID、ECID、AACUSTOMID 和 Analytics 源连接器
description: 了解 Analytics 源连接器如何处理 Adobe Analytics 身份标识字段。
...
AACUSTOMID 是一个单独的标识符字段，将根据 Analytics 实施中对 `s.VisitorID` 变量的使用，在 Adobe Analytics 中填充该字段。 在 Adobe Analytics 数据馈送中，AACUSTOMID 由 `cust_visid` 列表示。 如果存在 AACUSTOMID，则 AAID 将基于 AACUSTOMID。 （AACUSTOMID 优先于上述提到的操作顺序中定义的所有其他标识符。）

## Analytics 源连接器如何处理这些身份标识

Analytics 源连接器将这些身份标识以 XDM 形式传递到 Adobe Experience Platform，如下所示：

* `endUserIDs._experience.aaid.id`
* `endUserIDs._experience.mcid.id`
* `endUserIDs._experience.aacustomid.id`

这些字段未标记为身份标识。 相反，相同的标识将以键值对的形式复制到 XDM 的 **_identityMap_**，如下所示：
...
就 Customer Journey Analytics 而言，只有在您决定使用主要 ID 作为人员 ID 时，主要 ID 的定义才重要。 但这样做并不是强制性的。 您可以选择其他某个身份标识列作为人员 ID。
