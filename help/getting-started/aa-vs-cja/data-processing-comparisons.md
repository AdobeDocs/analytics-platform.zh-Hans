...
| 区段逻辑 | 评估事件/访问/人员（事件/会话/人员）区段规则。 | 示例：购买红色鞋款的顾客。 |
| 计算量度 | 评估客户创建的自定义量度，这些量度可以基于复杂的公式，包括区段。 | 示例：购买红色鞋子的人数。 |
