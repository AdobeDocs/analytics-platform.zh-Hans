...
* 实时数据或事件：当数据在 Adobe Experience Platform 中可用后，在 90 分钟内处理和摄取。（批次大小> 500万行：超过90分钟。） 如果启用了拼合，则摄取可能最多需要4小时。 有关详细信息，请参阅[护栏](https://experienceleague.adobe.com/en/docs/analytics-platform/using/technotes/guardrails)。
* 少量回填：七天内
* 大量回填：30 天内
